...
The current logarithmic \gls{matsim} utility function is not suitable for modeling activity sequence choice \citet[][p.127f]{Feil_PhDThesis_2010} and \citet[][]{MATSim_Userguide_2015}. Because of its negative second derivative, the log form favors very short activities, which means that the schedule is ever filled-up with short activities, where the usually long home activities are replaced first. Since this is completely unstable and unrealistic behavior, a new function was introduced by \citet[][p.129ff]{Feil_PhDThesis_2010}. The function was based on the asymmetrically S-shaped function proposed by \citet[][]{Joh_PhDThesis_2004}. Estimates of the new function based on the Swiss microcensus were provided. The estimation, however, suffered inherent stability issues as shown below. The function was, thus, not further developed.

% ----------------------------------------------------------
\subsubsection{Stability Issues With the S-Shaped Function}
Establishing a positive second derivative near point of origin is a natural idea to work against the degenerated mechanism of adding more and more and ever short activities.
The optimization problem differs from the log form optimization in the number of optimal points.
For the log form there is one optimal point which is is by $dt$ greater than zero where $\lim\limits_{dt \rightarrow 0}{}$. All first derivatives are equal at this point in line with consumer theory's utility maximization.

For an S-shaped function however, first derivative is bell-shaped, \ie not bijective. 
...
% ----------------------
\ah{Some pieces of notes to go there, might be used, when realizing that above is crap:
