...
Section~\ref{sec:future-frozen-randomness} indicates that further work is desirable to better understand the connection between \gls{matsim} scores and economic utilities.  At face value, Equation~(\ref{eq:utl-fct-w-frozen-rnd}) could be taken as each agent's utility.   As also discussed in Section~\ref{ch:economicEval:valuingBehavior:output2Eval}, problems arise when the $\tilde\epsilon_{ni}$ are not explicitly known for each person-alternative-pair $ni$.

In many past \gls{matsim} runs (those using \lstinline{Change/SelectExpBeta}), their effect was therefore parametrized by a logit choice model.  In these cases, the logsum of all plans' scores of an agent is each agent's correct utility measure.  This, however, requires that the logit model scale parameter is correctly estimated, and that each agent's set of plans is consistent with the estimation of the logit model.  Especially the second requirement is currently difficult to fulfill, see Section~\ref{sec:heterogeneity-in-plans-removal}.

In other \gls{matsim} runs (those using \lstinline{BestScore}), the $\tilde\epsilon_{ni}$ were effectively assumed to be zero.  In these cases, the highest of all plans' scores of an agent is each agent's correct utility measure.  Because of the \lstinline{BestScore} plan selection model, the plan with the highest score will at the same time also be the selected plan.

For some of us, it seems attractive to move into the direction of working with frozen randomness, as discussed in Section~\ref{sec:future-frozen-randomness}.  That approach would combine the advantages of the two approaches: It would inherit the parsimonious interpretation of the \lstinline{BestScore} approach, where only the plan with highest score ($=$ the selected plan) of each agent is considered, and at the same time include the effect of the $\epsilon_{ni}$.
