...
If supplied with a learning approach these within-day models should approach equilibrium after many iterations, as the agents with a suitable memory structure would avoid plans which make them worse off. This memory, which would need to be agent-specific and covering the very large set of choice options, makes the approach costly to implement. \kai{Meinem Verständnis nach greift obiges spieltheoretisch zu kurz.  MATSim sucht nach einem fixen Plan, der den höchsten Erwartungswert erzeugt.  Dies ist (bei stochastischen Systemen) \emph{nicht} identisch mit einem Regelsatz, der den höchsten Erwartungswert erzeugt ... z.B.\ könnte man beim Überqueren der Autobahn schauen, ob sie gestaut ist, und falls ja, auf der Landstraße weiterfahren.}
