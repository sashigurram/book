...
The core \gls{matsim} architecture---where agents learn utilities for plans---was originally derived from the field of \glsunset{cas}\acrlong{cas} \citep[CAS; \eg][]{AxelrodBook,Holland_1992,HraberJonesForrestEcho,PalmerEtAl_PhysicaD_1994} (also see Section~\ref{sec:streams} of this book). 
...
\gls{cas} focuses on many agents, agent interaction and emergence. \glsunset{ai}\Acrfull{ai} 
in contrast, concentrates on single agents. In \gls{ai} terms, the original \gls{matsim} agents (those doing day-to-day learning) are very simple 
%active 
...
%
For many aspects of \glsunset{its}\acrfull{its} systems, within-day replanning is indispensable \citep[e.g.,][]{hall-1993,EmmerinkEtAl_TransResC_1995,Dobler_PhDThesis_2013}.
%
...
It is thus important to include plans removal in all considerations,} since otherwise the very large $\beta^{remove}$ in Equation~(\ref{eq:6}), coming from always removing the worst plan, will dominate the statistical distribution.
...
The approach of \citet[also see Chapter~\ref{ch:destinationchoice} of this book]{Horni_PhDThesis_2013} can also be seen in this sense: attaching a random error term to each location-person-pair means that two persons---at exactly the same home location with exactly the same activity pattern---will select different locations for their activities.  So far, this describes heterogeneity between persons.  However, the approach also generates more heterogeneity per person, since the destinations attractive to each synthetic person will be spatially more spread out than they might otherwise be.
...
\label{eq:9}
\end{equation}
That is, replacing Equation~(\ref{eq:8}) by Equation~(\ref{eq:9}) in the \gls{matsim} scoring function would make, in first order, all activities equally likely to drop.  Starting with \gls{matsim} release~0.8.x, there will be a config switch 
\begin{lstlisting}{xml}
<param name="typicalDurationScoreComputation" value="..." />
\end{lstlisting}
where \lstinline{uniform} will mean the old behavior and \lstinline{relative} the behavior suggested in this section.  Consequences of this still need to be investigated.
...
It starts horizontally at zero duration, bends upwards with a positive second derivative and then changes curvature to the normal negative second derivative at longer durations.  
%
...
%
\citet{MoyoNagelptLineCalibration,MoyoNagelptNetCalibration} for access/egress times and transfers to \gls{pt},
%
...
As the above Section~\ref{sec:future-frozen-randomness} already indicates, further work is desirable to better understand the connection between \gls{matsim} scores and utility from consumer theory. At face value, Equation~(\ref{eq:utl-fct-w-frozen-rnd}) could be taken as each agent's utility. As also discussed in Section~\ref{ch:economicEval:valuingBehavior:output2Eval}, problems arise when the $\tilde\epsilon_{ni}$ are not explicitly known for each person-alternative-pair $ni$.

In many past \gls{matsim} studies, their effect has therefore been parametrized by a logit choice model (with the use of \lstinline{Change/SelectExpBeta}).  In these cases, the logsum of all plans' scores of an agent is each agent's correct utility measure.  See Section~\ref{ch:economicEval:valuingBehavior:output2Eval:logsum} for details.
%% This, however, requires that the logit model scale parameter is correctly estimated, and that each agent's set of plans is consistent with the one used for parameter estimation. 
%% %
%% \bktodo[inline]{Is that not the same? If the choice set is the same in estimation and in MATSim, then the scale parameter is simply 1.0, right?}
%% %
%% Especially the second requirement is currently difficult to fulfill, 
%% See Section~\ref{sec:heterogeneity-in-plans-removal} for 

In other \gls{matsim} studies, the $\tilde\epsilon_{ni}$ were effectively assumed to be zero (with the use of \lstinline{BestScore}).  In these cases, the highest of all plans' scores of an agent is each agent's correct utility measure.  Because of the \lstinline{BestScore} plan selection model, the plan with the highest score will at the same time also be the selected plan.  See Section~\ref{ch:economicEval:valuingBehavior:output2Eval:executed} to \ref{sec:economicEval:using-highest-score} for details.

For some of us, it seems attractive to move into the direction of working with frozen randomness, as discussed in Section~\ref{sec:future-frozen-randomness}.  That approach would combine the advantages of the two approaches from above: It would inherit the parsimonious interpretation of the \lstinline{BestScore} approach, where only the plan with highest score ($=$ the selected plan) of each agent needs to be considered, and at the same time include the idea of random utility theory, and, hence, the effect of the $\epsilon_{ni}$ on individual choices.
%
%% \bktodo[inline]{We do leave out any pointer to Gunnars interpretation here. Ok with me since we dont know whether this is a promising way. Frozen epsilons seem to be rather clear though...}

Another avenue of research is to further push the understanding of the econometric and statistical properties of the \gls{matsim} choice modeling, cf.~Section~\ref{sec:more-compl-variants}.
...
Apparently, social contacts, within households as well as within extended social networks, have a substantial influence on travel decisions, particular for social activities in leisure time \citep[][]{KowaldEtAl_ASNA_2009}.
An early social networks study, in context, but not based on \gls{matsim}, is by \citet[][]{MarchalNagel2005locationChoiceTrr}.
...
% KowaldEtAl_unpub_2009,%
KowaldEtAl_ASNA_2009}.
The most recent work on joint trips is reported in Chapter~\ref{ch:jointtrips}. 
