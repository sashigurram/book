...
This, however, is still not flexible enough.  \kai{Hier fehlt jetzt die Rechnung, dass die Verhältnisse der durations immer gleich bleiben ... muss m.E.\ noch nach part I.}

\kai{Baustelle}

Assume the usual \kai{chk} 
\[
S_{perf,i}(x) = \beta_{perf} \cdot t_{typ(i)} \cdot \ln(x/t_0) \ .
\]
Now assume that the day is spent on one long activity with $t_{typ(i)} = 20h$ and one short activity with $t_{typ(i)} = 4h$.  According to \kai{??}, the optimal time allocation will be 20h:4h.

Now assume that the person gets under time pressure.  We model this by making the day shorter, e.g.\ to 18h.  According to \kai{??}, the allocation will still be 20:4=5:1, i.e.\ 15h:3h.  
