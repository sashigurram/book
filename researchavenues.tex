...
The \gls{jdeqsim} (Section~\ref{sec:using-jdeqsim}) and the deprecated \gls{deqsim} (Section~\ref{sec:deqsim}) implement a double-queue model with backward traveling gaps.  Since recently, the \gls{qsim} now also has a double-queue variant \citep{AgarwalNagel2015Holes}, switched on by using a ``holes'' option in the config.
