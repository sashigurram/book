...
%
Part~\ref{part:extending-matsim} discusses various options to extend \gls{matsim} beyond these three elements, sometimes using only additional files, or using additional \gls{jar} files beyond the \gls{matsim} core \gls{jar} file, by writing so-called ``scripts in \gls{java}'', or by adding or replacing functionality.

\gls{matsim} writes configuration files in several locations; for example, in the logfile, in the iteration output directory, or with the \lstinline{CreateFullConfig} functionality described in Section~\ref{sec:lgs-config}.  As explained in Section~\ref{sec:survival}, these files come with comments explaining configuration options.  This is often the best source for configuration options.  %\kai{Andreas, meinem Eindruck nach geht der user guide hier auch nicht weiter als das Buch -- und soll es derzeit auch gar nicht.  Unsere Javadoc sagt m.E.\ traditionell sehr wenig bis gar nichts über config.} \ah{ok}
...
The \gls{configfile} section \lstinline|plans| specifies which population file with its day plans will be used (Section~\ref{sec:lgs-config} and \ref{sec:lgstarted-population-file}). Further configuration options, \eg specification of arbitrary agent attributes or subpopulations, are presented in Section~\ref{sec:extending-population}.

Further \gls{matsim} containers are described in Chapter~\ref{sec:extending-data-containers}.
...
Possibilities for extending the \lstinline|Controler| functionality are given in %Section~\ref{sec:extending-controler} and % da steht nichts mehr. kai, jan'15
...
Generally, computations using threads are not necessarily faster with more threads, which is also true for \gls{matsim}. Some experimentation is necessary for each combination of scenario and hardware.  Here are some recommendations:
...
\item The most sensitive parameter is that for the \gls{qsim}.
  %
  For somewhat older hardware (\eg Apple Macbook Pro from~2010), using all three remaining cores---in addition to the parallel event handling---led to negligible performance gains but left the machine useless for interactive tasks such as normal office work.
  %
...
An overview of \gls{matsim} mobility simulations is given by \citet[][]{Dobler_TechRep_IVT_2011}. %See also \citet[][]{Rieser_unpub_IVT_2011}.
...
%% The \gls{matsim} \gls{gui} (Figure~\ref{fig:matsimgui}) allows adaptation of these parameters with the command \lstinline|Tools...Create Sample Population|.
% bin immer noch der Meinung, dass das so nicht stimmt. kai, dec'15
%
%% \item
...
%\item
Currently \gls{qsim} is implemented as a single-queue 
%simulation queue 
model (see Chapter~\ref{ch:kinematicwaves}). Back-propagating gaps as discussed in Section~\ref{sec:trafficflowmodel} are available experimentally
(see Section~\ref{sec:researchavenues-double-queue-mobsim}) and configurable with the parameter
\begin{xml}
...
A somewhat ancient configuration parameter is the so-called stuck time.  It determines after how many seconds of non-movement a vehicle is moved across an intersection despite violating the storage constraint of the destination link.
%
This parameter was introduced to resolve grid-locks, i.e.\ geometrical arrangements where no vehicle can move any more.  With the \gls{qsim} model, it is possible to add vehicles beyond the storage constraint to an overcrowded link.  This corresponds to maintaining a minimal flow even under very congested conditions.
%
The default value of this parameter is set to 10, i.e.\ non-moving vehicles are moved forward after 10~simulation time steps of non-movement.  This may seem as a rather short time, but systematic investigations (unfortunately never published) have shown that the simulations become, in the comparison to traffic counts data, less realistic when this parameter is increased.  
...
\gls{jdeqsim} \citep[][]{WaraichEtAl_STRC_2009} was used for project \emph{KTI Frequencies} \citep[][]{BalmerEtAl_ResRep_datapuls_2010}. It is is a \gls{java} reimplementation of \gls{deqsim} \citep[][]{WaraichEtAl_STRC_2009,%
CharyparAxhausenEtAl2007Event-DrivenQueueBasedTraffic,%
%CharyparEtAl_TRR_2007,%
CharyparEtAl_TRB_2009} and provides parallel event handling, but no parallel simulation \citep[][p.11]{BalmerEtAl_ResRep_datapuls_2010}. Back-propagating gaps (Section~\ref{sec:trafficflowmodel}) are supported, but traffic lights, public transport and within-day replanning are not.
...
The \gls{configfile} section \lstinline|planCalcScore| specifies the parameters used for scoring agents' plans (Section~\ref{sec:lgs-config}); parameters are explained 
%in detail 
in Chapter~\ref{ch:scoring}.
...
% (neuer Satz, eingefügt nach Lektorat. kai, may'15)
%
%% Usually, so-called innovative modules do not define their own utility function,  particularly for random mutation modules, while best-response modules, such as destination innovation, are closer to choice modeling standard procedure, but still not full-blown choice models. 
% (habe ich mal rausgestrichen.  Es macht für mich irgendwie keinen Sinn, erst zu schreiben, dass innovative Module ``usually'' keine utl fct definieren (was ja auch nicht stimmt), nur um im zweiten Teil des Satzes dann ein solches zu nennen.  Und was davon näher an random utl modelling dran ist, ist irgendwie nochmal eine separate Frage. kai, may'15)
...
\end{xml}
%
Each module is given a weight determining the probability, by which the course of action represented by the module is taken. The strategy modules' weights are normalized, in case they do not sum to one. In this example, each agent changes her leg mode with probability~0.1 and her plan timing with probability 0.2. 
%% A strategy module is, in the code, always a combination of a plan selector and zero or more strategy module elements. 
%% In the example, the agent chooses a plan from her set of 
%% plans according to a logit model with probability~0.8. 
% (somehow garbled. kai, dec'15)
Otherwise, the agent chooses a plan from her set of 
plans according to a logit model.
...
In older versions of the \gls{configfile} you will find 
%following 
a deprecated configuration syntax using numbered strategy modules.
%
%% \begin{xml}
%% <module name="strategy" >
...
Please note that combining strategy modules that are \glspl{extension} (see Section~\ref{sec:levels-of-access}), like destination innovation together with public transport, 
%is complicated. 
may not always work as expected.  Combine them with care and contact the mailing list if you are unsure.
...
The module shifts activity end times randomly within a configurable range as described by \citet[][]{BalmerRaneyEtAl2005act-times, Raney_PhDThesis_2005}.
%% , Balmer_unpub_VSP_2004, BalmerEtAl_unpub_EIRASS_2004, BalmerEtAl_unpub_STRC_2004}. 
%
%all variants of the BalmerRaney paper that is already referenced. kai, aug'15

%% %A best-reponse approach to 
%%   time choice is applied by "Planomat" described in Section~\ref{sec:planomat}.

% ===================================================================================
\subsubsection{Route Innovation}
\label{sec:routechoice}

Route innovation is applied by 
%defining its parameters in the \gls{configfile} section \lstinline|planscalcroute|, by 
adding 
%
\begin{xml}
...
Mode innovation is applied by adding\footnote{%
%
The names may be changed into {\tt ChangeTripMode} and {\tt ChangeSingleTripMode}, please keep your eyes open.
%
}
%
\begin{xml}
<param name="strategyName" 
   value="{ChangeLegMode | ChangeSingleLegMode | SubtourModeChoice}" />
\end{xml}
%
...
\end{xml}
Starting with release 0.8.x, the \gls{configfile} comments give possible options.
...
% Haben wir das je untersucht? Ziehe vor, es wegzulassen. kai, dec'15
...
\end{xml}
%% The default is 0.8, i.e.\ innovation is switched off after 80\% of the iterations.  
It makes sense to use this together with \gls{msa} averaging of the scores (Section~\ref{sec:score-msa}).
...
It is currently not possible to generate different vehicle types from the config alone; one either needs to provide a vehicles file (see Section~\ref{sec:extending-vehicles} and Section~\ref{sec:vehicles-in-qsim}), or write a script-in-\gls{java} to generate the vehicle fleet (again see Section~\ref{sec:vehicles-in-qsim}).  
...
\item The dynamic transport systems contribution will eventually be able to move a \lstinline{taxicab} mode with taxis (see Chapter~\ref{ch:dts}). 
%% \atend{chk if dvrp contrib allows taxicab as mode}

%% \ah{Kam von, soweit ich sehe, von dir in Revision 1286 als ``The dynamic transport systems extension moves all modes that are registered with it with taxis. See Chapter~\pageref{ch:dts}.'' M.E. immernoch verständlich, auch wenn mir die PT-Geschichte vertrauter ist. Ist zwar im DTS-Kapitel nicht erwähnt, verschiedene Taxi-Modes sind aber denkbar (?), oder was meinst du?} 
%% %
%% \kai{Problem ist, dass das zwar naheliegend ist, aber leider derzeit so nicht funktioniert.}
%% %
%% \kaitodo{check}
