...
\lstinline|PassingQ| with vehicles of the same maximum speed still corresponds to \gls{fifo}, since vehicles with the same maximum speed will not pass each other in the \gls{qsim}.  Thus, for this and other reasons (e.g.\ vehicle-specific emissions calculations, see Chapter~\ref{ch:emissions}) it becomes necessary to assign different vehicle types to different persons, modes, or trips.

The arguably most ``honest'' approach in terms of micro-simulation is to generate a synthetic fleet of vehicles.  Each leg would then have to know which vehicle it wants to use.  This is indeed possible with the planned vehicle \gls{id} that \Gls{matsim} route objects can store.  This functionality is switched on by first loading an additional vehicles file (see Sec.~\ref{sec:extending-vehicles}), and then configuring the \gls{qsim} as
\begin{xml}
<module name="qsim">
...
An alternative vehicles source is
\begin{xml}
<module name="qsim">
...
Alternative approaches to vehicle generation currently need to be programmed as script-in-\gls{java}.  See Section~\ref{sec:extending-vehicles} for a reference to a script that generates mode-specific typical vehicles for each mode, and Section~\ref{sec:extending-qsim} for a general explanation of scripts-in-\gls{java}.
...
\paragraph{Vehicle Placement and Behavior}
...
A problem with either of the two approaches is that vehicles need to be at the correct location when the \gls{qsim} starts.  Here, the simulation currently places all vehicles at the \kaitodo{resolve in code!}
...
\item The dynamic transport systems contribution moves all registered modes \kaitodo{??} with taxis (see Chapter~\pageref{ch:dts}).
\ah{Kam von, soweit ich sehe, von dir in Revision 1286 als ``The dynamic transport systems extension moves all modes that are registered with it with taxis. See Chapter~\pageref{ch:dts}.'' M.E. immernoch verständlich, auch wenn mir die PT-Geschichte vertrauter ist. Ist zwar im DTS-Kapitel nicht erwähnt, verschiedene Taxi-Modes sind aber denkbar (?), oder was meinst du?} \kaitodo{check}
