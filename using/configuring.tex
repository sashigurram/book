...
The question now is how the teleportation routes are generated.  Clearly, the user can write her or is own router which can either be a full-blown network router (\cf Section~\ref{sec:router-extension-point}) or some heuristic that comes up with plausible travel times and travel distances.  It is, however, much easier to use the preconfigured options, as follows.
...
\end{xml}
means that if the router encounters a leg with mode pt, it should generate a ``teleportation'' route whose travel time is twice that of a freespeed car route.
\kai{what about distance?  and does this go into the leg or the route?}

\paragraph{Teleportation with teleported mode speed}  Setting, in the above, \lstinline{teleportedModeParameters} \kai{incomplete}
...
\ah{ab hier vielleicht anch Part II + forward pointer? Dann haben wir wieder Konsistenz.}
