...
\item The most sensitive parameter is for the \gls{qsim}.
  ...
  %
  Experiences with new servers are currently not available. \kai{or are there?  where?} \ah{Gemäss Patrick ist Sergio was am Basteln mit Desktop-Clustern.}
...
The PassingQ makes no difference as long as all vehicles have the same maximum speed.  It is therefore possible to define so-called ``mode vehicles'', i.e.\ a typical vehicle for each mode.  This is discussed \kai{where?}.\footnote{%
%  
\ah{ab hier fielleicht nach Part II + forward pointer?  Dann haben wir wieder Konsistenz.}
%
\kai{Ja das können wir versuchen.}
%
...
It is possible to give each planned route a list of possible vehicle \glspl{id} to be used for this route.  This is discussed \kai{where?}.


\subsubsection{So-called \gls{teleportation}}

All modes that are \emph{not} registered with the QSim as ``main modes'' will be teleported.  That is, the QSim will without problems process legs such as
\begin{lstlisting}
<leg mode="pedelec" >
...
In addition, the so-called matrix based pt router (Section~\ref{sec:matrix-based-pt-router}) uses a list of transit stops, and a matrix of stop-to-stop travel times and travel distances, and based on those informations computes a telported walk leg to the next stop, a teleported walk leg to the destination stop, and a telported walk leg to the final destination.
...
The matrix based pt router also serves as an example that, given the teleportation capability of the QSim, it is possible to come up with arbitrary algorithms for arbitrary modes as long as they generate (expected) travel times and (expected) travel distances.  As said earlier, the teleportation facility of the QSim will just use these two attributes at face value.  Although with such an approach neither congestion nor en-route replanning are or can be incluced, it is nevertheless a flexible approach that allows a fully modular addition of arbitrary modes without having to interact with the QSim.
