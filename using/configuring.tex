...
%
\kai{Andreas, ``Explicitly Simulated Passenger Modes'' ok?}
%
...
\end{xml}
{\color{red} The above configuration specifies that plans containing}
\begin{xml}
<leg mode="car"...>
\end{xml}
...
<leg mode="truck"...>
\end{xml}
{\color{red} will treated by the network router.}

As of the writing of this text, the router will route all the modes \ah{which ones? pt also?} \kai{Andreas, I added some lines above (in red).  Does that clarify it?} on the ``car'' links of the network.  Note that, per the network file \gls{dtd}, ``car'' is the default mode of each link as long as long as the link's mode field is not explicitly filled.
...
It is possible to register separate routers for specific modes.  The syntax for this is discussed in Section~\ref{sec:router-extension-point}.  The pre-configured extensions that were discussed in Section~\ref{sec:departure-handlers}---``multiModal'', public transit, taxis---come with corresponding routers.  
