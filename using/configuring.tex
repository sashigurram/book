...
%
\kai{``Explicitly Simulated Passenger Modes'' ok?}
%
...
\end{xml}
As of the writing of this text, the router will route all these modes \ah{which ones? pt also?} on the ``car'' links of the network.  Note that, per the network file \gls{dtd}, ``car'' is the default mode of each link as long as long as the link's mode field is not explicitly filled.
