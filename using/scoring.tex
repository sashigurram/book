...
\end{xml}
\kai{check possible re-name of monetaryDistanceCostRate}
...
% ====================================================================================
\subsection{Optimal activity durations}
\label{sec:optimal-activity-durations}

Let us now assume that a synthetic person wants to allocate durations $t_{dur,q}$ to activities $q$, and neither constraints nor travel are involved, i.e.\ the expression to be optimized is the sum of the logarithmic utility of performing, i.e.
\[
\sum_q \beta_{dur} \cdot t_{typ,q} \cdot \ln(t_{dur,q}/t_{0,q}) \ .
\]
This is an optimization under the time budget constraint that $\sum_q t_{dur,q} = 24\,h$, and therefore solved by the method of Lagrange multipliers.  

\createfigure{%
Computation of optimal activity durations
}{%
Method of Lagrange multipliers to obtain optimal activity durations
}{%
\label{fig:optimal-activity-durations}
}{%
\fbox{%
\begin{minipage}{0.98\hsize}
The Lagrangian is
\[
L := \sum_q \beta_{dur} \cdot t_{typ,q} \cdot \ln(t_{dur,q}/t_{0,q}) 
%
+ \mu \cdot \left( 24\,h - \sum_q t_{dur,q} \right) \ .
\]
The solution is obtained from
\[
0 \stackrel!= \frac{\partial L}{\partial t_{dur,q}}
= \beta_{dur} \cdot t_{typ,q} / t_{dur,q} - \mu 
\]
or
\[
t_{dur,q} = \frac{\beta_{dur} \, t_{typ,q}}{\mu} \ .
\]
$\mu$ could be obtained from the time budget constraint, but more important is that the above leads to
\[
\frac{t_{dur,p}}{t_{dur,q}} = \frac{t_{typ,p}}{t_{typ,q}} \ .
\]
\end{minipage}
}%end fbox
}{}

This is shown in Figure~\label{fig:optimal-activity-durations}; the important result for the purposes here is that, at optimal allocation, 
\begin{equation}
\frac{t_{dur,p}}{t_{dur,q}} = \frac{t_{typ,p}}{t_{typ,q}} \ .
\label{eq:optimal-time-allocation}
\end{equation}
That is, in the absence of travel and further constraints beyond the time budget constraint, time allocation between activities is optimal when the ratio of the actual durations is the same as the ratio of the typical durations.  If, say, $t_{typ,home} = 12\,h$, $t_{typ,work} = 8\,h$ and $t_{typ,shop} = 2\,h$, then the optimal time allocation between three activities of these three types will always follow the ratio $12:8:2 = 6:4:1$.

A more extensive treatment of time allocation can be found in Section~\ref{ch:economicEval:valuingBehavior}.
...
\kai{chk if default betaTrav was changed to zero when going to print} \kai{Siehe \url{https://matsim.atlassian.net/browse/MATSIM-335}}
...
In consequence, the Vickrey parameters of $\alpha$ (marginal penalty for arriving early), $\beta$ (penalty for traveling) and $\gamma$ (marginal penalty for arriving late) \citep[as defined by][]{ArnottEtcBottleneck-inelastic} are consistent with the following equations:
\begin{equation}
\begin{array}{ccrc}
...
\item Set $\beta_{tt,car} \equiv$ \verb$marginalUtilityOfTraveling...$ to $0.0$. (For release\,0.7.0, this is the default. \kai{chk})
