...
\hfill \textbf{Author:} ..., Andreas Horni, \ah{Kai Nagel?}
...
The extensions made over time to the original utility function are described in the next section~\ref{sec:utfextensions}. Clearly, only a few extensions made it into the default current utility function. \kai{I added a section ``Current version ...'' so wie can summarize which of the extensions actually made it into the default.}
...
\ah{Hier Schätzung von Kickhöfer erwähnen und etwas History wie man auf die Werte kommt?}
...
\ah{Irgendwo oben kurz darauf eingehen? Bei den Params?}
...
\ah{Im Singapore scenario angeben}
...
%
\ah{Ist im Carsharing Kapitel drin.}
...
\ah{Sollte ins Parkign Chapter rein}
...
\ah{Ich würde das lieber ganz hochnehmen, so dass man den current state ganz zu Beginn des Kapitels mitkriegt. Dafür würde ich "Extensions and replacements of the \acrshort{matsim} scoring function" sehr stark abspecken (Habe das mal entsprechend auskommentiert). Im Wesentlichen: Auch hier wieder History rausschmeissen. Die Extensions der UTF sind hier eigentlich sowieso am falschen Ort Platz (gehörten in den nächsten Teil.) Unabhängig vom Platz, finde ich den Hinweis, dass eine S-shaped function, error terms, facility load und agent-specific terms schon mal implementiert wurden doch wertvoll. Vielleicht ein zusätzliches Kapitel unter "Modules" einfügen und da die Erweiterung/Schätzung der UTF besprechen. Oder hier generell einen ganz kurzen "Pointer"'-Absatz mache im Stil von "UTF for Tel-Aviv cna be found in Section~\ref{sec:scenario.telaviv} etc.). \\
Ah, jetzt fällt mir gerade auch ein, dass Chef eine Zusammenstellung wie in obiger Section genau mal wollte.}
