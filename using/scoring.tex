...
The utility of an activity $q$ is calculated as follows \citep[see also][p.377ff]{CharyparNagel2005ga4acts}:
\begin{equation}
...
and $t_{0,q}$ is 
%the minimal duration, or in other words, 
the duration when utility starts to be positive. 
%
...
\item $S_{early.dp} = \left\{
  \begin{array}{l l}
    \beta_{early.dp} \cdot (t_{end,q} - t_{earliest.dp, q}) & \quad \text{if $t_{end,q} > t_{earliest.dp,q}$}\\
    0 & \quad \text{else}
  \end{array} \right.$
%
defines the penalty for staying not long enough, where $t_{end,q}$ is the activity ending time and $t_{earliest.dp,q}$ is the earliest possible activity end time $q$.  We normally recommend leaving $\beta_{early.dp}$ at zero, except if really good data about this effect is available.
...
%
Equation~(\ref{eq:tdisutility}) is the direct utility contribution of travel; see Section~\ref{sec:opport-cost-of-time} for the the full indirect utility as well as the relation to the \gls{vtts}, and Section~\ref{ch:economicEval} for a more general discussion.
...
$\Delta t_{trav}$, the person would not only gain from avoiding $\beta_{trav} \cdot \Delta t_{trav}$, but also from additional time for activities (effect of the so-called opportunity cost of time). The \textbf{(total) marginal utility of travel time savings} is thus:
%
\[
...
\]
where it can be imagined $q$ is the activity immediately following the trip (\cf Section~\ref{ch:economicEval:valuingBehavior}).
%
The marginal utility of travel time savings, $mUTTS$, can thus be defined as the indirect effect on the overall time budget, corrected by an offset $\beta_{trav}$ that denotes how much better, or worse, it is to spend that time travelling, rather than ``doing nothing''.\footnote{%
  %
...
can be transformed to the more common \textbf{\glsreset{vtts}\gls{vtts}} by division with the marginal utility of money, $\beta_{m}$:
\[
...
Arriving late incurs a marginal utility of $\beta_{late}$, typically negative.  Here, no additional opportunity cost of time is involved. Intuitively, arriving later implies having left the previous activity later.  That is: the current activity is shortened by the same amount that the previous activity was extended, leaving the overall score unaffected (\cf Section~\ref{ch:economicEval:valuingBehavior}). 

\paragraph{Vickrey Parameters}

As a result, the Vickrey parameters of $\alpha$ (marginal penalty for arriving early), $\beta$ (marginal penalty for traveling) and $\gamma$ (marginal penalty for arriving late) \citep[as defined by][]{ArnottEtcBottleneck-inelastic} are consistent with the following equations:
\begin{equation}
\begin{array}{ccrc}
...
it can happen that an agent arrives at the last activity of the plan at a later 24-hour-time than when the first activity ended.
...
%\kai{Nope.  Returning to the previous version.  Adding an example.}
For example, an agent could stay at home until 3am (end of first activity), then go through her daily plan including a very late party, and return home at 6am the next morning (Fig.~\ref{tab:negative-durations}).  In this case, the duration of the wrap around home activity would be \emph{minus} three hours.
...
\end{equation}
where $S^k$ is the newly memorized score, $S^{k-1}$ is the previously memorized score, $S$ is the score obtained from the plan's execution in the \gls{mobsim}, and $\alpha$ is a ``learning'' or ``blending'' parameter.  The default value of $\alpha$ is one; it can be configured by the line
\begin{xml}
...
\end{xml}
%% The default value is 0.8, meaning that \gls{msa} score averaging starts after 80\% of the iterations.  
% currently not default. :-(
This is plausibly used together with innovation switch off 
(Section~\ref{sec:innovation-switchoff}), meaning that \gls{msa} operates on a fixed set of plans.
...
\beta_{early.dp} & = & 0\, & utils/h .\\ % I checked; it is 0! kai, dec'15
  \end{array}
\label{eq:std-params}
\end{equation}
%% \atend{check $\beta_{trav, mode(q)}$}
They are very loosely based on the Vickrey bottleneck model \citep[e.g.][]{ArnottEtcBottleneck-inelastic}. 
...
%
Based on this, a possible approach to calibration is as follows:\footnote{%
%
...
\item Set $\betaperf \equiv$ \verb$performing$ to whatever the prefactor of car travel time is in your mode choice mode, while changing that parameter's sign from its typical $-$ to a $+$.
...
%% (For release\,0.8.x, this is the default.) % NOPE
...
If you do not have a mode choice logit model, set all $\beta_{tt,mode} \equiv$ \verb$marginalUtilityOfTraveling...$ values to zero (i.e.\ same as car).

\item Set distance cost rates \verb$monetaryDistanceRate...$ to plausible values, if you have them.
...
Calibrating schedule-based public transport (see Chapter~\ref{ch:pt}) goes beyond what can be provided here.
