...
\end{xml}
\kaitodo{at end: check possible re-name of monetaryDistanceCostRate}
...
\kaitodo{at end: chk if default betaTrav was changed to zero when going to print (siehe \url{https://matsim.atlassian.net/browse/MATSIM-335}}
...
\beta_{dur} & = & 6\, & utils/h \\
\beta_{trav, mode(q)} & = & 0 & utils/h \kaitodo{at end: check} \\
...
\label{eq:std-params}
\end{equation}
\kaitodo{at end: check $\beta_{trav, mode(q)}$}
...
\item Set $\beta_{tt,car} \equiv$ \verb$marginalUtilityOfTraveling...$ to $0.0$. (For release\,0.7.0, this is the default. \kaitodo{\atend chk})
