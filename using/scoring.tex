...
\kai{Nennen wir das jetzt doch wieder ``c''?  Wir hatten doch versucht, den Ausdruck ``cost'' loszuwerden, weil er in der Software mal als generalisierte Kosten und mal als monetäre Kosten gemeint war.  Chk}
...
\\ & & + (\beta_{d, mode(q)} + \beta_{c} \gamma_{d, mode(q)}) \cdot d_{trav,q} 
%
+ \beta_{transfer} \, x_{transfer,q} \, \\
\end{matrix}
\end{equation} 
where:
\begin{compactitem} 
...
\item $\beta_{transfer, mode(q)}$ are transfer penalties in public transport (normally negative) \kai{is this mode-specific?}, and
...
In consequence, the typical Vickrey parameters of $-6$, $-12$ and $-18$ \kai{citation} are obtained from the following equations:
\begin{equation}
\begin{matrix}
...
\citet[][p.393]{CharyparNagel2005ga4acts} come up with 
\begin{compactitem}
