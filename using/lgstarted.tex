...
% ##################################################################################################################
This chapter explains how to set up and run MATSim and it describes the requirements of building a basic scenario. Getting the source code into different computing environments and extending MATSim through the API is described in the second part of the book in Chapter~\ref{ch:extensionpoints}.

% ##################################################################################################################
\section{Setting Up MATSim}
Java SE 7 must be installed. The latest version of the Java Runtime Environment (JRE) can be downloaded from http://java.oracle.com. 


% ##################################################################################################################
\section{Running a MATSim Example Scenario}
MATSim does not provide a graphical user interface
...
% ##################################################################################################################
\section{Modules and External Tools}
externals tools (Via)

link modules chapter and say that:
Some modules are core modules and are set by default (such as the plans module), other modules can be plugged via the config file, and still others need to be additionally added via a listener in the MATSim controler. \ah{dahin referenzieren, wo dies erklärt wird.}
