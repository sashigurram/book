...
\gls{matsim} does not provide a graphical user interface, thus, you need to be able to handle and access a command line tool. In Linux or Mac OS X, this is typically a Terminal application, and in Windows, the Power Shell or Command Prompt.
...
\gls{matsim} creates a bunch of output data, which can be used to analyze the results but also to monitor the correct working of the current simulation setup. 
A \gls{matsimrun} contains a configurable number of \glspl{iteration}. \ah{vielleicht muss das noch an einen anderen Ort.}
...
The travel demand is usually derived from surveys; for Switzerland from the \gls{mz} \citep[][]{BfS-MZ2005_manual_2006}. Newer data sources, such as \gls{gps} or smartphone travel diaries might be an interesting future possibility.
