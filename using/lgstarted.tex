...
\end{compactitem}
%
With this setting, MATSim agents perform their activities on a specific \gls{link}. If further information about \gls{activity} locations needs to be specified, this can be done in 
\begin{compactitem}
...
The population data format is one of the most central data structures in MATSim and might be a bit overwhelming at first. Luckily, to get started, only a small subset must be known of it. Following listing shows how a minimal population file could look like. 
...
A \gls{matsimrun} contains a configurable number of \gls{iterations}. \ah{vielleicht muss das noch an einen anderen Ort.}
...
The following output files are contentiously build-up to summarize the complete run.
