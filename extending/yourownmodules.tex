...
\begin{compactitem}
\item A \lstinline$PlanStrategy$ picks one plan from memory according to a specified choice algorithm.
\item A \lstinline$PlanStrategy$ picks one plan from memory
at random, copies it, mutates it in some specific aspect, adds the mutated plan to the plan memory, and
marks this new plan as selected.
\end{compactitem}

MATSim provides a helper class which can be used to implement both of these strategy template. 
The helper class delegates to a \lstinline$PlanSelector$, which selects a plan from momory, and to zero,
one or more \lstinline$PlanStrategyModule$ implementations, which mutate a copy of the selected plan.
