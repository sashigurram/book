...
As common in research the universities groups' composition changes frequently. Over the last decade more than \ah{???} people build the MATSim core, where the recently founded Singapore group increases that number by another handful of persons. An overview of current and former MATSim team members can be found at \ah{.... todo: Overview page}.
...
Code documentation in form of \emph{Javadoc} can be found at \citet[][]{MATSIM-T-Javadoc_Webpage_2014}. 
...
The MATSim roles shown in Section~\ref{} lead to different ways of contributing to MATSim. 

Knowledge contribution as a user at user meeting. Additionally, there are external MATSim users, usually contributing to development by informing about their achievements for example in the MATSim user meeting, or also reporting back their needs or problems with the software, and sometimes bugs. 

Code contribution as an API-user or a developer

core contributions indirectly via core developers. If you want to become one, need to get member of the MATSim team. \ah{1-2 Zeilen, wie man Core-Developer wird. Ansonsten Core-Anfummeln ganz weglassen!}
 

In next Chapter ... technically explained how to hook to MATSim. im nächsten Kapitel hooks ... technically how ... and where
