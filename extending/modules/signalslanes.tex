...
In principle you can model each turn pocket as single link and simply place traffic signals at its end. 
...
In MATSim, all domain relevant attributes that differ from geospatial location are attached to links. 
...
Edges of the subgraph are called \emph{lanes}. 
Traffic flow on each lane is simulated nearly the same way as for links in the extended queue model. 
Just the calculation of minimum travel times slightly differs to avoid systematic errors of temporal resolution~\citep[][p.~38]{GawronPhd}. 
...
According to Cremer and Landenfeld (see \citep{CremerLandenfeld1998MesoTrafficSignalModel}), the vehicle is placed on the lane that currently contains the smallest number of other vehicles if there are several lanes leading to the same downstream link.
Note, that lanes in the model have no 1:1 relation to lanes existing on a link in reality.
