...
%%\createStandardInformationBasic{\url{http://matsim.org/javadoc} $\to$ core $\to$ signals}{The module is invoked by setting \lstinline|useSignalSystems| in the \lstinline|scenario| section of the config.}{Further parameters are set in the config module \lstinline|signalsystems|.}{\citet{GretherBischoffNagel2011CottbusSylviaEventAbstract,Grether_PhDThesis_2014}}

\createStandardInformation{signals}{\lstinline{RunSignalSystemsExample} class}{signalsystems}{\citet{GretherBischoffNagel2011CottbusSylviaEventAbstract,Grether_PhDThesis_2014}}
...
There are different strategies for traffic signal control: fixed-time traffic signal control, for example, periodically repeats the same schedule for signalization, while traffic-responsive signal control reacts dynamically to the prevailing traffic patterns to improve the junction or system performance.   
Traffic signal control can improve the traffic conditions at a single junction, but the whole system can be worse if a single junction is improved. 
...
If one wishes to capture signalization effects on a rather coarse level, consider the approach presented in~\citet[][pp.~139]{Charypar_PhDThesis_2008}, that can be realized with the time variant network feature of \gls{matsim} \citep{LaemmelGretherNagel2009TimeDependentNetworks}. 
...
This section summarizes results published in~\citet{GretherBischoffNagel2011CottbusSylviaEventAbstract,Grether_PhDThesis_2014}. Readers interested in details are referred to these publications. 
...
The diversity of traffic-responsive control algorithms is wide; for a review, see~\citet[][]{Grether_PhDThesis_2014}. 
...
\begin{description}\styleDescription
	...
	\item[Amber:] Specifies the amber phase at the beginning and end of green time. 
		Currently, driving is not permitted if a traffic signal group shows amber light and  
		this information is used only for visualization purposes. 
...
The shortest path calculation captures the effects of lanes without further modification \citep[see][pp.~21]{Grether_PhDThesis_2014}.  
...
Readers seeking more detail are referred to~\citet[][pp.~75]{Grether_PhDThesis_2014}.  
...
This chapter provides an overview of traffic signals in \gls{matsim}, detailing what to consider before taking first steps in larger scenarios. Details for implementation can be found in the \gls{javadoc} documentation referenced above. 
For the detailed scientific discussion of modeling aspects the reader is referred to \citet{Grether_PhDThesis_2014}.
