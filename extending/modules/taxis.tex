...
\createStandardInformationBasic{dvrp}{No predefined invocation. Setting up a \lstinline|VrpOptimizer| and coupling it with MATSim requires some tailor-made code as shown in Section~\ref{sec:dvrp_config}}{No typical MATSim config}
{
...
By doing that one can not only speedup computations significantly, but also avoid situations when %due to vehicles' inertia (e.g., an idle driver can stop waiting and depart only at the beginning of the simulation step),
...
The \lstinline$VrpOptimizer$'s performance may be assessed either by analyzing the resultant schedules, or by processing events collected during the simulation.
