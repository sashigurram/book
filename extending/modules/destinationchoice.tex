...
%
\kai{Habe das nochmal eingefügt.  M.E.\ steht der erste Teil so im Ben-Akiva, irgendwo im Umfeld von nested logit.  Das ist mir aber auch erst relativ spät halbwegs klar geworden, also sozusagen nachdem es bei Dir so funktioniert hat, ist mir klar geworden, dass die Theorie das genau so vorher sagt.}
...
Calibration is performed by scaling the error terms.} \kai{Andreas, hängst Du sehr an diesem Absatz?  Im BVWP-Umfeld gab es erbitterte Auseinandersetzungen, ob man die ``estimated'' oder die ``calibrated'' utl fct nehmen sollten.  Die Betreiber der Verkehrsmodelle argumentierten, dass eine Kalibrierung an beobachteten ($=$ revealed) Verkehrsströmen mindestens genauso gut seien wie eine Schätzung basierend auf stated preference.  Ich selber habe, wenigstens im Prinzip, große Sympathie für diesen Ansatz (was mir nicht passt, ist, dass diese Kalibrierung im BVWP-Prozess eine black box ist), und mindestens würde ich Daly unterstützen, der bei hEART-Stockholm sagte, dass die beiden Gruppen sich ``einigen'' müssten. -- Ich finde aber, dass das alles zu kompliziert ist für hier.}
