...
\createStandardInformation{locationchoice}{\lstinline|RunLocationChoiceBestResponse|}{locationchoice}{\citet[][]{HorniEtAl_unpub_TRB_2012, Horni_PhDThesis_2013}}
...
Due to the iterative base of \gls{matsim}, a mechanism ensuring consistent probabilistic choices over the course of iterations is required, which is the main component of the destination innovation module.
...
\footnote{Choice outcomes do not only depend on implemented choice model, but \st{are (naturally)} also \st{implicitly influenced by the implementation of the algorithm to find the solutions of the utility-maximization} \textcolor{orange}{on the algorithm to generate the} \textcolor{orange}{choice set}.  On the other hand, this is the basic idea of \gls{matsim} and thus not a critical concept. \kai{Den vorhergehenden Satz verstehe ich nicht wirklich.  Weglassen?}
}
...
Research about the necessity of error terms for the remaining choice dimensions is required. \kai{see research avenues?}
...
This reduces the storage space dramatically from $\bar{nq} \times nn \times n\ell$ to $\bar{nq}(nn + n\ell)$, where $nn$ is the number of persons or agents and $n_\ell$ is the number of destinations and $\bar{nq}$ is the average number of discretionary activities in an agent's plan.  \kai{Warum jetzt nicht $n_n$, $n_q$ und $n_\ell$? Und warum ``bar''?} 
...
This reduction of the search space saves a lot of computation time, however, it is still infeasible and further speed-ups are necessary. Most computation time is due to calculation of travel times, \ie due to routing, for evaluation of the alternatives in the search space. To reduce these huge routing costs, the Dijkstra\kai{?? references?} algorithm is not only applied forward---providing one-to-all travel times--but also \emph{backwards} using an average estimated arrival time as initial time. This is an approximation, and thus a \emph{probabilistic} best response is applied, justified by the 
%natural 
...
This means that choice sets and, hence, estimates are \st{highly} dependent on the modeler\st{, which is an exogenous problem, structurally similar to the well-known %\gls{maup} % does not work with \st{...}
...
In \citet[][]{HorniEtAl_TRR_2009}, based on the Zürich scenario, a \st{singly-constrained}\kai{wirklich ``singly''?  Im Sinne des 4 step processes wäre ``singly constrained'' $=$ es steht fest, in welcher Zone wie viele Leute losfahren.  Das ist bei matsim aber immer gegeben (solange die synthetischen Personen nicht zu Hause bleiben können).} model is presented that introduces competition for space-time slots on the activity infrastructure. 
The actual load is coupled with time-dependent capacity \st{re}{\orange con}straints for every activity location and incorporated explicitly into the agent's destination choice process as detailed below.  \kai{Meiner Intuition nach ist ``constraint'' und ``restraint'' nicht identisch: restraint $=$ Gummiband, constraint $=$ Drahtseil.  Es sieht unten tatsächlich eher nach ``restraint'' aus, aber dann ist es endgültig nicht (singly or doubly) contrained.} 
...
For destination innovation, this has led to a dramatic underestimation of total travel demand making inclusion of unobserved heterogeneity inevitable.\footnote{%
%
{\orange The problem clearly is that it is not possible to make all choices at the same level: Destination choice is conditional on mode choice which in turn is conditional on route choice, and it is well known that the randomness, expressed by the scale parameter, needs to be larger on the higher level decisions.  In that sense, the approach of the present chapter is to replace the need for more randomness in the choice model by directly including that randomness into the utility function. And that randomness needs to be quenched, since otherwise the iterative procedure will just average it out. \kai{Habe das nochmal eingefügt.  M.E.\ steht der erste Teil so im Ben-Akiva, irgendwo im Umfeld von nested logit.  }}
%
}
