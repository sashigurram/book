...
Instead, the random error terms $\varepsilon_{jl}$ need to remain fixed from iteration to iteration. The optimization is then performed as a deterministic search based on the resulting utilities $U_{jli}$, i.e., an alternative $l$ for person $j$ and activity $i$ is selected as 
\[ 
...
\item[(b)] Computing and storing a separate $\varepsilon_{jli}$ for every combination of person $j$, alternative~$l$ and activity $i$.
\end{itemize}
