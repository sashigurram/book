...
As explained in Section~\ref{sec:strategymodules}, in \gls{matsim}, we do not use the term \emph{choice} for the MATSim modules, that change transport choices, as the \gls{matsim} choice process differs from the discrete choice framework. The MATSim choice process is performed over the course of iterations, \ie it is based on the interplay of the innovation modules with the \gls{matsim} loop and and not on an isolated choice within a single module. 
