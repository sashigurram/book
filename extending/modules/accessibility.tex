...
% I (dz) I think the above citation is wrong.
...
%%%%%%%%%%%%%%%%%%%%%%%%%% for more details see Nicolai Nagel
...
%%%################################################################
%%% include figure for NMBM here?
%%%################################################################

%To use this functionality a \texttt{GridBasedAccessibilityControllerListenerV3} needs to be added to the controller.
%Alternatively, if the calculation based on some given zonation system is desidered, the 
%\texttt{ZoneBasedAccessibilityControllerListenerV3} can be used.
%
%BuettnerEtAl2010Erreichbarkeitsatlas, p.49
...
%##########################################################################################################################
%following rather directly adapted from NicolaiNagel
...
%
%Opportunity locations such as work places are given by land-use. As stated earlier, they are attached to the nearest network node. 
...
This assumption is unlikely to alter accessibility results significantly, but offers huge potential for the
optimization of computational performance, which has---as stated by many authors
%##########################################
%cite Buettner, Kwan
---often been a major obstacle to higher-resolved accessibility computations. In the concrete case of the
\gls{matsim} accessibility computation, the exploration of the entire network by the ``least cost path tree''
is a computationally expensive task.

Thanks to the aforementioned assumption that opportunity locations are attached to the nearest network \emph{node},
...
to be done individually for each origin $i$. \citet{Nicolai} show that due to this runtime optimzation, computation 
time increases sub-linearly with resolution. At the same time, they find that no significant further insights can be 
gained by increasing the resolution beyond 100\,meters.
...
\section{Data}
\mnote{data}
As mentioned above, the availabiliy of data is mostly the main determinant for the lvel of spatial resolution
...
The accessibility extension of \gls{matsim} can, however, be run on data from \gls{osm} which contains data on
activitiy facilities on a coordinate-based level, \ie using these data it is not necessary to rely on a zonation
system. Furthermore, data form \glx{osm} is publicly and freely availbale and its amount and quality of these
data are steadily increasing. A
