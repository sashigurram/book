...
%
%In contrast, for the zone-based approach the accessibility of the measuring point is taken as representative for the whole zone.  In consequence, the choice of the procedure of how to generate the measuring point for the zone-based approach has an influence on the results.  There is no corresponding choice for the grid-based approach, removing one element of arbitrariness compared to the zone-based approach.

The following paragraphs concentrate on the cell-based approach.
%%  that is
%% qualified for high resolution accessibility calculations. 
Nevertheless, the
...
%%The sum of travel disutility $V_{jk}$ to get from a node $j$ to
%%opportunity $k$ over all opportunities $k$ that are aggregated on the same node
%%are included. If the travel disutility from origin $i$ to node $j$ is
%%$V_{ij}$ and the disutility from node $j$ to opportunity $k$ is $V_{jk}$. 
Then 
\begin{equation}
\sum_{k \in j} e^{ V_{ik}} 
%
...
This is an exact result. The only approximation was already done
