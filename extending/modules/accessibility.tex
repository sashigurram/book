...
infrastructure elements, such as public transport facilities, to make them accessibile, i.e. useable 
...
%MorrisEtAl1978AccessibilityIndicators
...
in terms of those accessibility components, which are not the particular focus of the given kind of measure. Accordingly, \citet{Geurs2004AccessibilityReview} assert that it would be desireable if an accessibility measure included 
...
because they are likely to offer different kﬂinds of medical treatment that complement each other. 
...
%product of the destination's attractiveness and the ease to get there.
...
The most simple approach to calculate a travel time 
...
%The model is essentially a destination-based accessibility model run using Arc- GIS software
...
%(################# link to the RunAccessibilityComputation class already mentioned).

By doing so, the transport and land-use compoents are well-represented.
network
facilities.
...
are relevant, as opportunities may become unavailbale to citizens if they cannot be reached anymore due to overly high 
...
%the expressiveness of the accessibility calculation can be significantly increased by taking into account 
%(transport and activity) demand next to the consideration of the supply side (\ie the transport infrastructure supply 
%and the supply with spatially distributed activity facilities). 
...
%
%absence of planning support tools that can inform decision-making about public trans- port networks
%
%p. 455f
%New forms of transport analysis are emerg- ing that can be used, in addition to the tradi- tional transport models, 
...
%destination-based ac- cessibility modelling has expanded rapidly with the coalescing of advanced GIS, com- prehensive 
...
%
%While these new tools, like the traditional transport models, sit within a positivist framework, they expand the field of positiv- ist analysis in new directions. They also rely on a logical and mathematical treatment to provide scientific knowledge
%
%The application of each tool has intersected with transport and land- use decision-making, attempting to reveal insights that would influence decision-makers to consider alternative transport futures
