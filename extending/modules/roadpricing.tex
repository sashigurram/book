...
\\
\url{http://matsim.org/javadoc} $\to$ roadpricing

\textbf{Invoking the module:}
\\
 \url{http://matsim.org/javadoc} $\to$ roadpricing $\to$ \verb$RoadPricingExample$ class

\textbf{Configuration:}
\\
\verb$roadpricing$ section of the config file (only available when the roadpricing extension is correctly loaded).
...
%\who{Rieser, Chakirov, Grether, Kickhöfer, Nagel}

% ##################################################################################################################
\section{Introduction}
\label{sec:roadpricing-intro}

Roadpricing is an often-discussed policy measure \citep[e.g.][]{...} \kai{todo}.  Its implementation in \acrshort{matsim} is conceptually straightforward \citep{%
%
...
As an early illustration of the capabilities, an afternoon toll for the Zürich area was simulated.  While this is not the most probable policy scheme, it still clearly demonstrated the advantage of the integrated approach over other approaches: Not only did the synthetic travelers switch to public transit, but they also did so for the morning rush hour, where no toll was charged.  Thus, the \acrshort{matsim} approach proved its capability to affect the whole daily plan, not just the trip.  For more information, see \cite{RieserEtAl_TRBTDF_2008}.
