...
Maximum speed on any of these links is calculated based on distance and flight duration provided by OAG. 
Times for taxi, take-off, and landing are also taken into account, i.e.,\, flight duration is reduced by the time needed from push-back to airborne before the maximum speed for an airway link is calculated.
...
The assignment of flights to the desired \gls{od} connection, i.e.,\,the passenger routing, is calculated by \gls{matsim} 's default public transit routing module.
...
Each virtual passenger can now choose between the micro-simulated air transport options and an alternative mode. 
...
Instead of this deterministic plan removal, a probabilistic algorithm can be implemented: e.g.,\,plans for removal can be selected based on a path size logit model. 
...
%\mnote{Extent}
All the presented modeling approaches explain passenger routing in more detail than itechnically possible from the input data.  
...
The current approach would not find such a solution, since passengers do not consider costs they impose on others; see~\citet{LaemmelFloetteroed2009KISysOptEvac} for an approach taking that into account.  
...
In principle, other data, e.g.,\,Lorenz curves and median incomes, can be merged with the \gls{od} data~\citep{KickhoeferEtAl2011PolicyEvaluationIncome}.  
...
In the same way as~\citet{ClarkeEtAl2007AirNetworkSim}, further models for, e.g.,\,gates, taxiing, weather or airline operations can be added to the approach. 
