...
% ##################################################################################################################
Options for the simulation of air- and rail-transport technology and passengers using \glsentrytext{matsim} are subject of this chapter. 
...
This chapter presents an approach, how \glsentrytext{matsim} can be applied to capture these constraints and how the interaction between a passenger demand and the constraints on the technology supply can be modeled. 
The public transit model of \glsentrytext{matsim} (Chapter~\ref{ch:pt}) is applied. %public transit vehicles and stops. 
...
Especially the choice set generation and plans removal algorithm of \glsentrytext{matsim} is discussed, that is also subject of Section~\ref{sec:future-of-scoring-function}. 
...
%\mnote{Network}
The modeling of the air network aims at a simulation with \glsentrytext{matsim}.  
...
The flight schedule is taken from the OAG data and translated to a \glsentrytext{matsim} transit schedule containing information about each line, route, and departure. 
...
The passenger demand for trips in Germany created and used for the results of this section is based on \glsentrytext{od} data of DESTATIS 
\footnote{\url{destatis.de}, Fachserie 8 Reihe 6, last access 10.09.2012}.
% 
For each \glsentrytext{od} pair and trip a virtual person is created.
...
The assignment of concrete flights to the desired \glsentrytext{od} connection, i.e.,\,the passenger routing, is calculated by the default public transit routing module of \glsentrytext{matsim}.
...
Each passenger chooses out of a set of 5\,plans using an \glsentrytext{mnl}.
The outcome is stable after 500\,iterations, thus departure time choice and routing are switched off. 
For another 100\,iterations only the \glsentrytext{mnl} is used by the passengers to select a plan. 
%
% ------------
\createfigure%
...
The travel time, $tt$, is computed by the microsimulation with an estimation of the beeline distance between the \glsentrytext{od} pair $d$ and a velocity $v$, i.e.,\,$tt = d / v$.  
...
The synthetic population is generated based on \glsentrytext{od} pairs that may contain transfers ($od_{transfers}$), 
...
For comparison, the number of passengers on direct flights is calculated for each \glsentrytext{od} pair ($sim_{direct}$) from the simulation results.
...
whereby $|OD|$ denotes the number of \glsentrytext{od} pairs, $sim_{direct}(i)$ the simulated passengers on a direct flight between the \glsentrytext{od} pair $i$, and $od_{direct}(i)$ the same, but retrieved from data.  
With the same values, the (unsigned) mean relative error for each \glsentrytext{od} relation is calculated as
$
...
E.g., average speed and temporal availability of train connections depends on the \glsentrytext{od} pair. 
In principle, the alternative mode could be refined by inclusion of \glsentrytext{od} pair dependent average speed data. 
...
The data for the demand provides \glsentrytext{od} pairs for air transport, but not for car, train or bus trips.  
...
Passengers get more likely stuck on \glsentrytext{od} pairs where the demand excesses seat capacity. 
...
Assuming that the number of available seats is increased accordingly, the simulation model provided too little capacity, at least on certain \glsentrytext{od} pairs. 
...
Otherwise, a longer time horizon could be simulated\footnote{Note, that this requires some changes in the source code that may not be resolved by sole customizations of \glsentrytext{matsim}. Please ask the developers before running \glsentrytext{matsim} for a longer time horizon.}. 
...
As the present model is based on sole \glsentrytext{od} data, it does not include such a process. 
In principle, other data, as e.g.,\,Lorenz curves and median incomes, can be merged with the \glsentrytext{od} data~\citep{KickhoeferEtAl2011PolicyEvaluationIncome}.  
...
As the open source code of \glsentrytext{matsim} comes with options for extension, more detailed models of the technology side hinge on the availability of data. 
...
The advice to the interested user is to support the \glsentrytext{matsim} team in solving these more general questions first.  
