...
{(Iterative) within-day replanning MATSim loop \kai{I think this figure needs to come earlier, i.e.\ in Sec.~\ref{sec:SimulationApproaches}.}}%
...
In the present implementation, their \emph{random number generator} is re-initialized for every replanned agent using a deterministic value (e.g.~a combination of the agent's Id and the current time step).  This ensures that an agent's decisions can be reproduced even when the global sequence of random numbers changs. \kai{Christoph, ist es das?  ``have to'' (wie Du es vorher hattest), erscheint mir auf jeden Fall zu stark.  Oder hat es etwas mit multi-threading zu tun?}
...
In consequence, the code uses a copy of the original plan and modifies the copy.  The score, however, is given to the original plan.  The implementation is free to \kai{Christoph: ``is able to''? (existiert das derzeit als Option?} \emph{also} memorize the executed plan and add it to the set of plans.
...
One example where setting the original to the executed plan clearly does not make snese is parking search \kai{citation?}.
...
\item \verb$Id chooseNextLinkId()$ -- agent is asked at intersections and needs to return how to proceed.

%% \item \verb$Id<Link> getDestinationLinkId()$ -- agent is essentially asked if it wants to arrive on the current link.\footnote{%
...
\item \verb$void notifyMoveOverNode(Id newLinkId)$ -- agent is notified that it has traversed the intersection and entered a new link.
...
\end{lstlisting}
\verb$Guidance$ helps the agent with making decisions, see below.
