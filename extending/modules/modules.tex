...
See \javadoc\ $\to$ main distribution $\to$ \lstinline{RunSubpopulationsExample} class for an example.  That example looks like coding in \gls{java} is necessary, but this is really not the case; \gls{java} is just used to generate the populations, which could as well be done by other means.
...
%
There used to be an additional ``useVehicles'' config switch.  In release 0.7.0, that switch will be gone.
%
}
...
For a variety of reasons---e.g.\ vehicle-specific emissions calculations (Chapter~\ref{ch:emissions}), or vehicle-specific maximum speeds (see below)---it becomes necessary to assign different vehicle types to different persons, modes, or trips.
%
