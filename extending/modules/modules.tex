...
% ...........................................................................
\paragraph{Replanning Modules:} Package \lstinline|org.matsim.core.replanning.modules| provides replanning functionality that can also be plugged into MATSim. 
...
Through multiple engines it can be run in parallel. The simulation is able to handle time-variant networks, within-day replanning \citep[][]{Dobler_TechRep_IVT_2009}, and in an experimental manner also traffic lights \citep[][]{Neumann_MastersThesis_2008}).

% -----------------------------------------------------
\subsubsection{Multi-Modal Simulation}
\label{sec:multimodalsim_qsim}
\gls{qsim} can run multi-modal simulations. Public transport can be included as described in \citet[][]{Rieser_PhDThesis_2010} and Chapter~\ref{ch:pt}. 
\ah{Also here I am not up to date: are pt-car etc. interactions possible with a truly inter-modal network?}

Interactions between modes can be included by specifying the parameters \lstinline|linkDynamics| and \lstinline|mainMode|. For link dynamics either \gls{fifo} can be chosen or an approach, where passing is possible. i.e.,\,\lstinline$<param name="linkDynamics" value="FIFO|PassingQ" />$. Parameter \lstinline|mainMode| defines, which modes are congested. Technically, these are the modes that the departure handler of the \lstinline|netsimengine| handles. Effective cell size, effective lane width, flow capacity factor, and storage capacity factor need to be set with diligence. Theoretically also pedestrians can be included, however, only vehicular modes actually make sense. Thus also the vehicles and their type need to be specified for multi-modal interactions. \ah{where? in transit.\lstinline|<param name="vehiclesFile" value="null" />|}.

Simulation experiments for this relatively new feature have been performed for the Patna scenario (Section~\ref{sec:patna}). 

In Chapter~\ref{ch:multimodalsim}, an earlier approach to improve the teleportation travel time estimation is presented. This approximate approach lies in between \gls{fifo} and passing.


\ah{notes:} \\
1) Scenario - Include vehicle types and vehicles corresponding to each person in scenario. \ah{params? }
2) Config - Set link dynamics from available options (Passing or FIFO) and inform qsim and router \ah{where? \lstinline|<param name="networkModes" value="car,ride" />|?} about main congested modes.
