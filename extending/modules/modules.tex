...
% ====================================================================================================
MATSim follows a modular concept.
...
%
} and in consequence modules can exist at many levels in a software framework.
...
\item Click on the \gls{matsim} jar file\footnote{This works since winter 2014/15 and should be in the 0.7.x~release.} and follow the instructions. 
\end{itemize}
...
Clearly, a plug-in architecture had to be the solution, but it took (and still takes) time and effort to make the extension points both sufficiently capable and sufficiently robust.  
...
%Due to 
As a consequence of the distributed and project- and dissertation-driven \gls{matsim} contribution process (see Chapter~\ref{ch:developmentprocess}), modules are often implemented for a specific practical purpose, leading to various limitations of the respective module. For example, modules might only work for a specific mode, or for a defined calling order.  Normally, additional effort needs to be undertaken to generalize the module;
...
% gibt es nicht mehr. kai, jul'15
	...
	Analysis & Contrib & Section~\ref{sec:contrib-analysis} \\
	...
	Car Sharing & Contrib & Chapter~\ref{ch:carsharing} \\
	Joint Trips and Social Networks & Contrib & Chapter~\ref{ch:jointtrips} \\
	...
	Signals and Lanes & Contrib & Chapter~\ref{ch:signalslanes} \\
	PSim & Contrib & Chapter~\ref{ch:psim} \\
	Cadyts & Contrib & Chapter~\ref{ch:cadyts} \\
	Belief Desire Intention (BDI) Framework & Contrib & Chapter~\ref{ch:bdi} \\
	wagonSim & Contrib & Chapter~\ref{ch:wagonSim} \\
	freightChainsFromTravelDiaries & Contrib & Section~\ref{sec:freightChainsFromTravelDiaries} \\
	matrix-based pt router & Contrib & Section~\ref{sec:matrix-based-pt-router} \\
	\hline
...
Alternatively, network change events can be directly added to the code.
...
A powerful extension of a standard population can be achieved by specifying further agent attributes in a \lstinline|ObjectAttributes| file input to \gls{matsim} by parameter \lstinline|inputPersonAttributesFile|. 
...
See \javadoc\ $\to$ main distribution $\to$ \lstinline{RunSubpopulationsExample} class for an example.  That example looks like coding in \gls{java} is necessary, but this is really not the case; \gls{java} is just used to generate the subpopulations, which could as well be done by other means.
...
Facilities are an optional element of \gls{matsim}; some \glspl{module}, such as the destination innovation module (Chapter~\ref{ch:destinationchoice}), depend on it, however.
...
\end{xml}
%
Besides the activities that can be done in the facility, further location attributes, such as opening times, can be specified. 
...
%
There used to be an additional ``useHouseholds'' config switch.  In relase 0.7.x, that switch will be gone.
%
}

Loading the households file does not mean that it is used anywhere in the code; such functionalities needs to be switched on separately.  Currently, no such functionality can be switched on from the \gls{configfile} alone.
...
%
There used to be an additional ``useVehicles'' config switch.  In release 0.7.x, that switch will be gone.
%
}
...
Just loading the vehicles does not use them; that needs to be configured separately.  See Section \ref{sec:vehicles-in-qsim} for details.
...
The controler module, can be extended by \lstinline|ControlerEvents| and \lstinline|ControlerListeners| as detailed in Chapter~\ref{ch:extensionpoints}. 
...
For a variety of reasons---e.g.\ vehicle-specific emissions calculations (Chapter~\ref{ch:emissions}), or vehicle-specific maximum speeds (see below)---it may become necessary to assign different vehicle types to different persons, modes, or trips.
%
The arguably most ``honest'' approach to vehicles in terms of micro-simulation is to generate a synthetic fleet of vehicles.  Each leg would then have to know which vehicle it wants to use.  This is indeed possible with the planned vehicle \gls{id} that \Gls{matsim} route objects can store.  This functionality is switched on by first loading an additional vehicles file (see Sec.~\ref{sec:extending-vehicles}), and then configuring the \gls{qsim} as
\begin{xml}
<module name="qsim">
...
\end{xml}
(available with release 0.7.x).  This states that every time the \gls{qsim} needs a vehicle with a specific \gls{id}, it will search for it in the vehicles data container, throwing an exception if it is not found there.  
...
\end{xml}
This generates a default vehicle (typically a medium-sized 4-seater) every time a vehicle is needed.  This is currently the default configuration.

Alternative approaches to vehicle generation currently need to be programmed as script-in-\gls{java}.  
%% As an alternative, there exists the possibility to use \lstinline|VehicleUtils| for specifying mode vehicles according to the example syntax in 
See, e.g., 
\lstinline{RunMobsimWithMultipleModeVehiclesExample} under \url{http://matsim.org/javadoc} $\to$ core
...
\end{xml}
This means that if a necessary vehicle is not available at the location where it is needed by the traveler, the \gls{qsim} throws an exception and aborts.  The idea here is that such synthetic travellers should have within-day replanning strategies (see Sec.~\ref{ch:withinday}) to cope with unexpectedly unavailable vehicles, and any attempt to use an unavailable vehicle points to an error in the behavioral logic of the driver.
...
\end{xml}
may be an option.  Here, a driver will wait if a vehicle is not available.  Errors in the coordination logic, i.e.\ very long waits, will be punished via the \gls{matsim} scoring logic, and thus lead to more robust coordinations.
...
Both with vehicle behavior of type \lstinline{exception} and of type \lstinline{wait}, vehicles need to be at the correct location when the \gls{qsim} starts.  Here, the default simulation currently places all vehicles at the home location -- for other variants, some additional code needs to be written or used, such as the car sharing extension (Chapter~\ref{ch:carsharing}).
...
The simulation is able to handle time-variant networks \citep{LaemmelGretherNagel2009TimeDependentNetworks}, within-day replanning \citep[][see Chapter~\ref{ch:withinday}]{Dobler_TechRep_IVT_2009}, and traffic lights \citep[][see Chapter~\ref{ch:signalslanes}]{Neumann_MastersThesis_2008,GretherNeumannNagel2011TrafficLightsMatsim,GretherNeumannNagel2012SignalsQueueModel}).
%
