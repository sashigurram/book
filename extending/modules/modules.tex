...
\atend{re-read modulex.tex just before going to print}
...
Also the population container was already discussed earlier, in Section~\ref{sec:using-population}.
%
...
Note that loading a facilities file into \gls{matsim} by itself does not use them; the functionality needs to be switched on by other means.  Currently, this is only possible by using some class with a \lstinline{main} method that does that.
...
\lstinline{Scenario} is a super-container that contains all the other data containers, accessible for example as \lstinline{scenario.getNetwork()}.  It used to have configuration options, but these are all gone now, so \lstinline{Scenario} is only visible once you are programming in \gls{java}. 
...
The simulation is able to handle time-variant networks \citep{LaemmelGretherNagel2009TimeDependentNetworks}, within-day replanning \citep[][see Chapter~\ref{ch:withinday}]{Dobler_TechRep_IVT_2009}, and traffic lights \citep[][see Chapter~\ref{ch:signalslanes}]{Neumann2008DA,GretherNeumannNagel2011TrafficLightsMatsim,GretherNeumannNagel2012SignalsQueueModel}).
%
