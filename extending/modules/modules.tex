...
Just loading the vehicles does not use them; that needs to be configured separately.  For this, it 

In addition, the qsim needs to be configured with 
\begin{xml}
   <param name="vehiclesSource" value="fromVehiclesFile" />
\end{xml}
(starting with release 0.7.0).  This means that \kaitodo{???}
...
%}
%%
%\kai{I THINK that I refactored the code ... but cannot remember this with certainty over the vacation.  Wenn Amits Bemerkung älter ist, dann stimmt sie nicht mehr.}
%\ah{Aussage von Amit war vorher, in Mailkonversation über multi-modale QSim.}

As an alternative, there exists the possibility to use \lstinline|VehicleUtils| for specifying mode vehicles according to the example syntax in \lstinline{RunMobsimWithMultipleModeVehiclesExample} under \url{http://matsim.org/javadoc} $\to$ core (see also Section~\ref{sec:using-qsim-multimodal}). 
...
The simulation is able to handle time-variant networks \citep{LaemmelGretherNagel2009TimeDependentNetworks}, within-day replanning \citep[][]{Dobler_TechRep_IVT_2009}, and traffic lights \citep[][]{Neumann_MastersThesis_2008,GretherNeumannNagel2011TrafficLightsMatsim,GretherNeumannNagel2012SignalsQueueModel}).
...
%% \begin{itemize}\styleItemize 
%% \item by specifying a vehicles input file
%% \begin{xml}
