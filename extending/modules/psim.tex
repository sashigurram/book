...
The greatest current performance limitation to \gls{matsim} is the network loading simulation, \ie the \gls{mobsim}, for example \gls{qsim} or \gls{jdeqsim}. 
This chapter is focused on \gls{qsim}. 
...
With the multi-modeling approach \citep[][]{FourieEtAl_TRR_2013}, shown in Figure~\ref{fig:PSim}, a \gls{matsim} run periodically replaces \gls{qsim} for a number of iterations with a simplified meta-model or \gls{psim} that runs approximately two orders of magnitude faster. 
