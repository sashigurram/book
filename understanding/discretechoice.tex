...
One observes in \MyEqRef{eq:unconditional-choice-proba} that the behavior of an agent depends on both the choice model ($P_{n}(i\mid C_{n})$), and the way in which the choice set is generated ($P_{n}(C_{n}\mid \mathcal{U}_n)$). The following two sections will look at each step in more detail.
...
%
\benjamin{Re-think title (coming from Kai, I guess...what does `this' refer to? Maybe just `Illustrative Example'?)}
...
%
\benjamin{Ich finde diese Absätze hier sehr gut. Hier konkret ist komisch, dass zunächst steht, dass `folgende Interpretationen gegeben werden können', und im folgenden Satz steht, dass `dies schwierig ist'. Re-phrase?}
%
...
%
\gunnar{Hier k\"onnte man jetzt zitieren und, wenn es denn passt, kurz umreissen, was da passiert, oder auf das betreffende Modul in Teil 2 verweisen.}
%
\benjamin{Gute Idee.}
%
Possible biases induced by these procedures are, however, rather difficult to identify.
%
\benjamin{Wirklich? Identifizert haben wir sie ja schon (s. den Issue-Teil von mir, der jetzt wieder im Economic-Eval Kapitel steht). Quantifizieren ist schon schwieriger...re-phrase?}

It therefore appears worthwhile to revisit the plan choice set simulation
%
\benjamin{Simulation der Generation? Ist das dasselbe? Sonst haben wir immer Generation oder Innovation verwendet...}
%
...
%
\benjamin{Habe den pointer mal eingefügt. Fraglich ist dann, ob man die default time mutation für deinen Ansatz durch ein best-response module ersetzen müsste? Und: wie Kai dort anmerkt, ist `tatsächliches best-response halt eigentlich nicht möglich'...}
%
...
durch ... ... (andreas: see source (Kompilierfehler!))
%(\ref{eq:limiting-accept-proba}) 
die konkrete Zielverteilung ... (andreas: see source (Kompilierfehler!))
%(\ref{eq:global-choice-model}) 
realisiert.}
%
