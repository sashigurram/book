...
The expected maximum utility of the model \MyEqRef{eq:mixture-of-logit} is of
...
\end{eqnarray}
with $\gamma_{i}^{r+1}$ being a random variable that captures the
variability of $\eta$ from iteration to iteration. This means that
the $\eta_{i}^{r+1}$ experienced in iteration $r+1$ differs by $\gamma_{i}^{r+1}$
from the $\eta_{i}^{r}$ value that was assumed by the agent when
having chosen plan $i$. 

In order to end up at the expected experienced maximum utility, one
hence needs to add (an estimator of) the expectation of $\gamma_{i(r)}^{r}$
to \MyEqRef{eq:wartezimmer}, with $i(r)$ being the chosen alternative
$i(r)$. Using \MyEqRef{eq:score-random-walk} and \MyEqRef{eq:score},
one obtains
\begin{eqnarray}
...
utility:
\begin{eqnarray}
\text{E}_{\eta}\left\{ \ln\sum_{i}e^{V_{i}+\eta_{i}}\right\}  & \approx & \frac{1}{R}\sum_{r=1}^{R}\ln\sum_{i}e^{V_{i}+\eta_{i}(r)}+\frac{1}{R}\sum_{r=1}^{R}\left(S_{i(r)}^{r+1}-S_{i(r)}^{r}\right).\label{eq:wartezimmer-corrected}
\end{eqnarray}
