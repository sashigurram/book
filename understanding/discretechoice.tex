...
and \citet{Train_2003} \corr{wrote}{are} the two standard textbooks in this
area.
...
%However, 
Both perspectives lead to the same result, \corr{}{the} %adaptation 
...
by agent $n$ and \corr{$C_{n}$ will}{let $C_{n}$} denote that agent's
...
of the concrete choice set then results from \corr{adding}{summing up} the
probabilities of selecting it in the presence of all possible choice sets $C_{n}\subset \mathcal{U}_n$.

It is evident in \MyEqRef{eq:unconditional-choice-proba} that an agent's behavior depends on 
...
$P_{n}(i\mid C_{n})$ for given choice sets. In \gls{matsim}, \corr{a plan's
score is evaluated and plan selection is}{a plan is evaluated and selected}
based on the score as \corr{}{the} sole property of the plan. This is only a
technical specification; \corr{}{the} scoring and selection protocols are
responsible for representing adequate perceptional and behavioral mechanisms. 
The notions of {}``choice''
and {}``selection'' are \corr{}{subsequently} used interchangeably (\cf
...
according to \corr{the}{} \glsreset{msa}\gls{msa}
...
Without the particular configuration mentioned in the previous section, things
are somewhat more complicated.
Assume that the attribute vector $\mathbf{x}_{ni}$ of the alternatives in
\MyEqRef{eq:ExpBetaPlanSelector} is defined through (a transformation of) the
...
variable\corr{,}{ and} not just an expected value, one obtains a
...
density function of \corr{}{the} random disturbances of all alternatives of
individual $n$
...
normal\corr{,}{} because this already allows \corr{}{for the} introduction of
...
Additional complexity results from \corr{}{the} simulated network conditions
being, in turn, the consequence of simulated travel behavior that is again
defined through \MyEqRef{eq:mixture-of-logit}.
% % This circular % dependency is natural, given that \gls{matsim} is mainly a
% transport planning % model (see Chapter~\ref{ch:montecarlo}).
% % % % \benjamin{Dies verstehe ich nicht, klingt ein wenig abwertend. Wieso
% muss das in einer Transportsimulation so sein? Wie wäre es denn wenn MATSim
% eine Börsensimulation wäre? Ähnlich oder? Ansonsten sehr schön formuliert, man
% versteht, dass wir auch nicht wissen, was alles in $\boldsymbol{\eta}_n$
% drinsteckt :)} % % % Indeed,
...
\begin{enumerate}
\item Assume one could somehow make \corr{}{the} simulated network conditions
...
would also increase the realism of \corr{}{the} resulting choice distribution.
...
\corr{postulated behavioral model random error terms}{the error terms in the
behavioral model} are indeed mobility simulation outputs. Simulated travel time
...
to combine a sequence of past network conditions \corr{to make an}{into an}
instantaneous $\boldsymbol{\eta}_n$ realization. 
Without a sound instance of such a learning model,
a \corr{behavioral justification}{learning-based interpretation} of
\MyEqRef{eq:mixture-of-logit} cannot be given.

Another perspective on this problem is possible, continuing
the arguments of Chapter~\ref{ch:montecarlo}. \corr{It is argued there that one
can consider the 
purpose of MATSim's iterative mechanism to attain a realistic
stationary distribution.}{It is stated there that the purpose of 
MATSim's iterative mechanism is merely to attain a realistic
stationary distribution.} If so,
...
Assuming -- for simplicity -- that \corr{on convergence, the unfiltered
stationary score $S_{ni}^{\infty}$ fluctuates}{the unfiltered
stationary score $S^{\infty}$ fluctuates in stationary conditions}
...
\end{eqnarray}
\corr{$\bar{S}$}{This means that the filtered score} is unbiased
\corr{for}{with respect to} the underlying score process and \corr{}{that} its
variance is in the interval from zero to \corr{$\text{VAR}\{S\}$}{the variance
of the unfiltered score}, depending on the chosen $\alpha$.
...
estimated from real data, just like any other model parameter\corr{l}{}.
...
relevant to the \corr{MATSim output microeconomic interpretation}
{microeconomic interpretation of MATSim outputs}.
\corr{Its computation formula}{A recipe for its computation} is described next. 
%
Let
\begin{eqnarray}
...
utility depending on \corr{}{the} scores at the time of plan selection,
...
and $\text{E}_{\eta}$ represent expectations \corr{for}{with respect to}
$\varepsilon$ and $\eta$, respectively. 
The remaining
\corr{expectation argument}{argument of the expectation} is the expected maximum
...
This expression holds\corr{,}{} regardless of the \corr{mobsim-generated mixture
distribution functional form}{functional form of the mobsim-generated mixture
distribution}.

Now, one needs to account for the fact that agents can only evaluate 
...
The expectation \corr{for}{of} this quantity can again be approximated by
...
network condition variability\corr{: for instance, like}{, for instance along
the lines of} the last paragraphs of Section~\ref{sec:more-general-choice}.
...
The choice set of agents can in principle be \corr{pre-computed and then be fed
into the MATSim simulation, technically using MATSim as a choice model
(Section~\\ref\{sec:Evaluating-choice-models\})}{computed a priori and then held
fixed during a MATSim simulation run}.
...
As Equation~(\ref{eq:unconditional-choice-proba}) \corr{at the beginning of this
chapter showed}{shows}, the generation of \corr{}{the} choice sets \corr{will
always affect}{affects the} simulated choices.
The simplest illustration of this mechanism is\corr{:}{that} alternatives that
...
very different from \corr{alternatives used}{the alternatives considered} in the
real world, it is unlikely that the simulation will display correct aggregated
quantities \corr{and even more unlikely that the simulation will provide}{or}
useful sensitivities for policy measures.

These types of biases are \corr{fully acknowledged}{well-known} in the discrete
choice community, even though \corr{their focus is}{the focus is there},
...
An interpretation of mutation and innovation as \emph{perceptional models} of
...
frejinger-2009}. The problem appears to be \corr{even}{} less explored when it
...
problem, \corr{allowing for}{aiming at} valid predictions at the system level.
%  \gunnar{Benjamin, wenn Du hier etwas zitiert wissen m\"ochtest, dann
% erg\"anze bitte die Referenzen.}  \benjamin{Gute Idee.}
Possible biases induced by these procedures can, however, be difficult to
quantify.
%  \benjamin{Wirklich? Identifizert haben wir sie ja schon (s. den Issue-Teil
% von mir, der jetzt wieder im Economic-Eval Kapitel steht). Quantifizieren ist
% schon schwieriger...re-phrase?}
...
theoretical foundation in the simulation context\corr{;}{.}

\corr{it}{It} thus seems worthwhile to revisit the plan choice set generation
%
...
mindset\corr{,}{} than \corr{to}{} deliver a complete solution.
...
% \gls{matsim} performs in every iteration one of the following operations for
% each agent: (1) update of the choice set and (2) choice of a plan from the
% updated choice set, using the model in
% Equation~(\ref{eq:ExpBetaPlanSelector}).
...
a formulation of \gls{matsim} \corr{for}{as} such a chain, the \gls{mh}
...
To make this concrete, let \corr{us define}{} the state space of the algorithm
% is defined
\corr{as}{be} the tuple $(C_{n},\, i\in C_{n})$ consisting of choice set and
...
Assume that the plan innovation yields exactly one new plan $i_{\text{in}}$
through a
% an approximative or exact
best response algorithm
% \benjamin{Ist ``best-response'' irgendwo definiert? Wäre an dieser Stelle
% extrem hilfreich.} \gunnar{Ja, das sollte wohl schon in Teil 1 auftauchen,
% z.B. im Zusammenhang mit den ``replanning strategies''.}
...
% Glossar bekommen.)}  \benjamin{Habe den pointer mal eingefügt. Fraglich ist
% dann, ob man die default time mutation für deinen Ansatz durch ein
% best-response module ersetzen müsste? Und: wie Kai dort anmerkt, ist
% `tatsächliches best-response halt eigentlich nicht möglich'...}
\corr{The}{Let the} corresponding plan innovation distribution \corr{is}{be}
approximated by $e^{\mu_{\text{inno}}S_{ni}}/\sum_{j\in
\mathcal{U}_n}e^{\mu_{\text{inno}}S_{nj}}$\corr{,}{} with a very large
$\mu_{\text{inno}}$. \corr{Assuming}{Assume} further that $i_{\text{in}}$
replaces exactly one uniformly selected plan $i_{\text{out}}$, which implies
that the choice set size $J$ is constant and \corr{excluding,}{exclude} for
simplicity\corr{,}{} the case that the best response innovation reconstructs the
removed plan exactly. \corr{from this, one obtains}{This leads to}
\begin{eqnarray}
...
thus corrected \corr{}{for} by accepting the new plan\corr{,}{} only if it
...
%
\corr{because}{in the sense that} the plan choice set innovation process
...
\corr{}{the} underlying plan choice sets are generated\corr{}{,} and different
...
on discrete choice modeling, \corr{one should ensure that  
agent behavior simulation is consistent with a particular discrete choice
model}{one needs to be careful when assuming full consistency with a
particular discrete choice model}. 
