...
The expected maximum utility of \MyEqRef{eq:mixture-of-logit} is of
...
Now one needs to account for the fact that agents evaluate past scores
when making a choice that leads to a future score payoff.
Recalling that this score variability is represented by the $\eta$ variables 
in \MyEqRef{eq:mixture-of-logit},
\begin{eqnarray}
\eta_{i}^{r} & = & \eta_{i}^{r-1}+\gamma_{i}^{r}\label{eq:score-random-walk}
\end{eqnarray}
is written with $\gamma$ being a random variable that captures the
variability of $\eta$ from one iteration to the next.

\gunnar{Need to improve notation: The above implies right now that an agent
only uses $\eta_{i}^{r-1}$ as a predictor of $\eta_{i}^{r}$. To avoid speculation about
the learning mechanism (which indeed is nothing to make concrete here), one
should use something like $\hat{\eta}_{i}^{r}=\varphi(\text{``past
observations''})$. -- Anyway, as long as one can observe both predictor and
future realization in the simulation, the computational recipe can still be applied.}

In order to express the expected maximum \emph{experienced}
utility, one hence needs to add (an estimator of) the expectation of $\gamma_{i(r)}^{r}$
to \MyEqRef{eq:wartezimmer}, with $i(r)$ being the alternative chosen in
iteration $r$.
...
& \approx &
\frac{1}{R}\sum_{r=1}^{R}\ln\sum_{i\in C_n}e^{S_i^{r-1}}
+
\frac{1}{R}\sum_{r=1}^{R}\left(S_{i(r)}^{r}-S_{i(r)}^{r-1}\right).
...
A further microeconomic interpretation of this quantity is postponed to
