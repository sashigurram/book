...
The product of these terms thus represents the probability that\corr{:}{} 
...
meaning that, for the remaining fraction of iterations, scores will be averaged
...
being, in turn, the consequence of simulated travel behavior that
is again defined through \MyEqRef{eq:mixture-of-logit}. 
%% This circular
...
would also increase the realism of \corr{}{the} resulting choice distributions.
...
\corr{postulated behavioral model random error terms}{error terms in the
...
stationary score $S_{ni}^{\infty}$ fluctuates in stationary conditions}
...
\end{eqnarray}
\corr{}{This means that the filtered score} $\bar{S}$ is unbiased
\corr{for}{with respect to} the underlying score process and \corr{}{that} its
variance is in the interval from zero to $\text{VAR}\{S\}$, depending on the chosen $\alpha$. 
...
(Section~\\ref\{sec:Evaluating-choice-models\})}{computed a priori and then
held fixed during a MATSim simulation run}.
%
...
As Equation~(\ref{eq:unconditional-choice-proba}) at the beginning of this chapter showed, 
the generation of \corr{}{the} choice sets \corr{will always affect}{affects
...
$\mu_{\text{inno}}$. Assuming further that $i_{\text{in}}$
replaces exactly one uniformly selected plan $i_{\text{out}}$, which implies
that the choice set size $J$ is constant and excluding\corr{,}{} for
simplicity\corr{,}{} the case that the best response innovation reconstructs the
removed plan exactly, \corr{from this, one obtains}{one obtains}
\begin{eqnarray}
...
thus corrected by accepting the new plan\corr{,}{} only if it is\corr{,}{} by chance\corr{,}{}
