...
%\bktodo[inline]{Check github diff...}
...
%% \ah{M.E. ist an dieser Stelle $U$ nun hergeleitet ... anders als in Teil I und II.} \kai{Ist es dann $U$ oder $V$?  Also sind die epsilons schon drin (ah locachoice) oder nicht (bryan raney betaBrain)?  Würde m.E.\ dafür sprechen, hier lieber bei $S$ zu bleiben ...  Oder halt doch $U$, aber eine Fußnote dran.}\ah{Fussnote unten enthält (jetzt?) die Aussage schon. Fussnote hochziehen?}
%% \bktodo[inline]{check $U$ vs. $V$}
%
...
%
%\bktodo[inline]{From proof-reading: ``
%%
%; for example, reducing travel duration by $\Delta t_q$ affects not only $U_{trav,q}$, but will also lead to a utility change of $ \mu \cdot \Delta t_q$---from the constraint---which can be interpreted as the linearized utility effect of spending that time otherwise.''
%}
%
...
%\bktodo[inline]{From proof-reading: ``
%%
%In contrast to time utility above, we do \emph{not} break down marginal utility of spending money for travel into a marginal utility of money as a resource, plus an ``offset'' for spending money on that particular purpose \citep[for an example of this decomposition, see, e.g.][]{MunizagaEtAl2012TimeAllocationModeChoice}.  Because there is no monetary budget constraint, there is also no ``neutral'' Lagrange multiplier that would give the marginal utility of money as a resource.''
%}
...
%\bktodo[inline]{From proof-reading: ``
%%
%At this point, we assume that the $V_i$ scale is already sufficient to absorb the the scale parameter effect.''
%}
...
\label{sec:economicEval:using-highest-score}
%As the last alternative to be discussed here, 
...
%\bktodo[inline]{Check this section...DONE}

Section~\ref{sec:more-general-choice} discusses the idea that \gls{matsim}'s typical choice model might be described by a mixture-of-logit model.  In that model,  $\epsilon_{ni}$ remains fixed per agent $n$ and alternative $i$, but other attributes such as the network conditions vary from one iteration to the next.  
%
...
%\bktodo[inline]{From proof-reading: ``
%%
%Values from literature should only be used if behavioral model values are not available. This can, however, potentially limit project comparability in different regions of the same state, or in different member states of the \gls{eu}.''
%}
%
...
One goal of this section~\ref{ch:economicEval:aggregatingValues} is to show the impact of a possible integration between behavioral modeling and economic evaluation in the same agent-based framework. 
...
%\bktodo[inline]{Add \citet{JaraDiaz2007TransportEconomicTheory} and SVTTS vs VTTS...}
%
...
%
%\bktodo[inline]{From proof-reading: This does not make sense...}

One option would be simply provide time equivalents, \ie the \gls{bca} would return time equivalents per invested monetary unit.
%
...
%
%willingness-to-pay of the population considered in order to realize the project.
%
...
%
%willingness-to-forgo-time of the population considered in order to realize the project.
%
%\bktodo[inline]{From proof-reading: ``
%%
%equivalent of lifetime hours that would be generated by the project for the population under consideration.
%}
%
...
%
%\bktodo[inline]{Add something from here to ResearchAvenues?}
%
...
If one is interested in monetization, \ie converting utility values into monetary terms, then additionally the marginal utility of money as a resource ($\lambda$ in the text above, and $\beta_m$ in Section~\ref{sec:typicalParams}) needs to be known. 
%
