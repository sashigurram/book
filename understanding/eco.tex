...
%
Alternatively, \acrshort{matsim} can generate choice sets of agents within the iterative loop
%
\benjamin{Some reference to the loop.}%
%
...
model.\footnote{%
%
A possible approach to increase the diversity in route choice is investigated by \citet{NagelKickhoeferJoubertHeterogeneousVoTs}.
%
}
...
%
are fixed.Then in each iteration each agent makes a draw from its choice set according to its scores, based on the choice model(s) defined in the configuration file (see Chapter~\ref{??}).\footnote{%
%
...
%
(see Chapter~\ref{ch:monteCarlo})\footnote{%
%
...
%
Such an approach can currently not be configured for \acrshort{matsim}; it needs to be programmed explicitly in Java. This has been performed for location choice (see Chapter~\ref{???}).
