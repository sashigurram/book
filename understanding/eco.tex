...
\gunnar{Ich habe hier jetzt diverse Kommentare eingef\"ugt, aber nichts geschrieben/ver\"andert. H\"atte das gerne
vorher abgestimmt. M\"ochte auch darauf hinweisen, dass ich hier mangels Sachkenntnis auf nichts bestehe.}
...
\gunnar{$\righttarrow$ Das hier befindet sich jetzt schon im ``discrete choice''-Kapitel. $\rightarrow$}
...
\label{eq:acts}
\end{eqnarray}
and the time constraint equation from above. \gunnar{Perhaps explain the meaning of U-dash as a time derivative.}
%
...
where we are using the indirect utility function \gunnar{Ist das jetzt eine der zuvor angegebenen Funktionen?} since we assume that the traveler compares optimal allocations before and after the change.
...
%
In principle, 
\gunnar{Vielleicht besser: ``Under the assumption of a correctly specified model and choice set''}
...
%
} In this interpretation, the \gunnar{expected} \acrshort{matsim} score $S_i$ is equated with the systematic part of the utility $V_i$.
...
However, as described in the previous Section~\ref{ch:economicEval:describingBehavior}, the use of \acrshort{matsim} as choice set generator yields issues with incompleteness of the choice set and with correlation 
...
ich nochmal nachrechnen, steht aber vermutlich auch irgendwo bei Train.}
