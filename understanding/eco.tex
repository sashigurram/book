...
However, since the following text uses terms such as ``marginal utility of time'' or ``marginal utility of money'', the equations are also noted using $U$ instead of $S$. 
%
}
...
Alternatively, one could simply use the highest score that the agent has in its plan.  This would only make sense if the true behavior is assumed to always select the plan with the highest score.  Again, this should then also be expressed by the choice model, \ie using the highest score only makes sense when the agent always selects the plan with the highest score, in which case the result becomes the same as Section~\ref{ch:economicEval:valuingBehavior:output2Eval:executed} and \ref{ch:economicEval:valuingBehavior:output2Eval:average}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{More Complicated Variants}

Section~\ref{sec:more-general-choice} discusses that choices might be made according to a mixture-of-logit model.  In that variant, $\epsilon_{ni}$ remains fixed per agent $n$ and alternative $i$, but other attributes such as the network conditions vary from one iteration to the next.  
%
...
It is, however, clear that this will not result in optimal average agent behavior -- the agent may be pushed towards some choice by a random fluctuation of the $\vec\eta_{ni}^k$, but obtaining a much lower score from that choice in the average.  Overall, the agent would be better off by first averaging the score of each alternative over many iterations, and then base her choice on those scores.  This, however, goes back to the converged scores of Section~\ref{sec:score-is-converged}.

Note, however, that this becomes rather involved: We postulate that the agent sees the full \gls{matsim} score, plus some private $\epsilon$s; that she optimizes based on the sum; that the \gls{matsim} model, however, does not know the $\epsilon$s and thus sample from the logit model; but that the economic utility has to include the effect of the $\epsilon$s although we do not know them, as in Section~\ref{ch:economicEval:valuingBehavior:output2Eval:logsum}.  Overall, thus, assigning utility values to such behavior as described by Equation~(\ref{eq:mixture-of-logit}) needs a better understanding of the underlying behavioral rationality.  Section~\ref{sec:future-frozen-randomness} discusses this a bit further.


\bktodo{Benjamin, could you please read this section to be informed about its existence?  Thanks ... Kai}

%% \paragraph{Recalling Standard Logit for Comparison}

%% Selecting $j$ such that
%% \[
%% U_i = V_i + \epsilon_i
%% \]
...
For simplification now write
\[
U_i = V_i + \eta_i + \epsilon_i \ ,
\]
where, to remember, $\epsilon$ is random but fixed while $\eta$ varies from one draw to the next.

Quite plausibly, if the agent makes a decision based on maximizing $V + \eta + \epsilon$ rather than $V + \epsilon$, then the expected utility is
\[
\ln \sum_i \exp( V_i + \eta_i ) \ .
\]

However, the utility returned by option $i$ after the draw is not $\eta_i$, but $\eta'_i \equiv \eta_i + \gamma$.  We can measure $\gamma$ one way or the other by
\[
\gamma = \eta'_i - \eta_i = [V'_i + \eta'_i] - [V_i + \eta_i] = S'_i - S_i
\]
between iterations.
...
\todokai{expected utility of mixture-of-logit unclear}
