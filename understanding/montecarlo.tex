...
With respect to \gls{matsim}, the following holds: \kai{@Gunnar: Das habe ich aufgrund unsere Diskussionen neu versucht; schaust Du es bitte durch?  Danke.}
\begin{itemize}
...
In the case of the config option \lstinline|ChangeExpBeta|, instead a Gibbs sampling approach is used.  Thus, iterations are correlated by the sampling process, but the process still ensures that the system samples from the \gunnar{same} underlying distribution.
\gunnar{Ich weiss nicht, ob das dann Gibbs ist, es kann aber sein.}
