...
and supply \corr{should be}{are} mutually consistent, in the sense that network
...
conditions $S^{k}$. As a result, \corr{}{the} information $Z^{k}$ is now
...
Step~\ref{enu:All-travelers-execute} runs \corr{}{the} the (stochastic) mobility
...
In symbols, let $X^{k}$ be the Markov
...
$Z$. Of these, only \corr{}{the} information $Z$ qualifies as a state variable:
if one knows $Z^{k}$, it is possible to draw the next day's travel
demand $D^{k+1}$\corr{,}{} based on \MyEqRef{eq:choice-model}, to insert this
...
because an evaluation of \MyEqRef{eq:learning-model} is impossible
...
particular \corr{given information $y$sequence, resulting travel demand
...
patterns that $Z^k$ from \MyEqRef{eq:learning-model} remains the same as
...
Examples are the} plan choice random utility model or the randomized selection
...
\end{eqnarray}
This expression merely sums up all simulation \corr{probability outputs}{output
probabilities} that exceed the threshold.
\item ...
\end{itemize}
...
Now recall \corr{initially asking certain questions about
simulation outputs}{that initially certain questions about
simulation outputs were asked}. The first row \MyEqRef{eq:define-question}
represents exactly these questions in a formal way---and the last
row \MyEqRef{eq:approximate-answer} provides a simple method for computing
...
such a chain, with its state space comprised of \corr{nformation}{the
information} available for replanning. This representation was exploited to
