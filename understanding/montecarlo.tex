...
not belong to transport modeling standard terminology. This
chapter explains MATSim function in modeling and simulation concepts
more established in the transportation field.
...
certain travel behavior chosen by all travelers in the system. Over
this, comes the modeling assumption that demand and supply should
be mutually consistent, in the sense that network conditions resulting
from certain travel behavior are statistically equal to the network
...
$S$ to the collection of all events 
...
\end{equation},
meaning that iteration $k$travel demand follows a probability
distribution conditional on information $Z^{k-1}$
...
distribution conditional on $D^{k}$ demand. 

Step~\ref{enu:All-travelers-observe} updates (possibly stochastically)
information available to all agents using the new network
...
specifies at least some of the information update logic behind $P(Z\mid C^{k})$. A greater level of detail is, however, not necessary in this chapter.
...
models. Further, one iteration of this process requires only information
...
in one particular state $y$, then entering $x$. The overall probability
of arriving in $x$ results from summing up these probabilities.
...
are: demand $D$, network condition $S$ and information
...
particular given information $y$sequence, resulting travel demand
...
that the concrete iteration where one evaluates the simulation
...
(iii) mobility simulation creates identical congestion and travel time patterns 
...
The above is a rather formal argument; more intuitively, the stochasticity in \gls{matsim} means that, even if the system returns multiple times exactly to a state where it has been before, it is improbable that it does so in the same number of steps. 
...
Many of the models used in \gls{matsim} are stochastic: for instance, plan choice random
utility models or the randomized selection of the
...
\end{eqnarray}
This expression merely sums up all simulation probability outputs  
...
Now recall initially asking certain questions about
simulation outputs. The first row \MyEqRef{eq:define-question}
...
fixed choice sets, then it could probably be feasible to compute the above
...
such a chain, with its state space comprised of nformation
