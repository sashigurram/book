...
{}``Agents'' that {}``learn'' in a {}``synthetic reality'' is a common term
...
not belong to \corr{transport modeling standard terminology}{the standard
terminology of transport modeling}.
This chapter explains \corr{MATSim function in modeling and simulation concepts
more established in the transportation field}{the functioning of MATSim in
terms of modeling and simulation concepts that are more established in the
...
MATSim, both kinds of feedback are strong \corr{modeling drivers}{drivers of
the modeling}.
...
given their information about \corr{}{the} network conditions.
...
\corr{Over
this, comes the}{This is complemented by the} modeling assumption that demand
and supply \corr{should be}{are} mutually consistent\corr{,}{} in the sense that
\corr{}{the} network conditions resulting from \corr{}{a} certain travel
...
that \corr{the}{} there is more than a superficial resemblance between the
...
The remainder of this chapter phrases these statements more technically and
explains their implications for \corr{}{the interpretation of} \gls{matsim}
\corr{output interpretation}{outputs}.
This presentation is \corr{, in part,}{in parts} a more technical reformulation
...
travel behavior, resulting in \corr{}{the} travel demand $D^{k}$ of the entire
agent population.
\item \label{enu:All-travelers-execute}All agents execute their travel
plans, resulting in \corr{}{the} (time-of-day dependent) network conditions
$C^{k}$.
\item \label{enu:All-travelers-observe}All agents see the resulting network
conditions $C^{k}$. As a result, \corr{}{the} information $Z^{k}$ is now
available to \corr{}{all} agents.
\end{enumerate}
...
placeholders for arbitrarily large \corr{data and complex structures}{and
...
\end{equation}\corr{,}{}
meaning that \corr{iteration $k$travel demand follows a probability
distribution conditional on information}
{the travel demand of iteration $k$ follows a probability distribution
that is conditional on the information} $Z^{k-1}$
available to \corr{}{the} agents at the end of iteration $k-1$.
...
probability distribution \corr{conditional on $D^{k}$ demand}{that is
conditional on the demand $D^{k}$}.

Step~\ref{enu:All-travelers-observe} updates \corr{}{the} (possibly
\corr{stochastically}{stochastic}) information available to all agents
...
In symbols, let $X^{k}$ be the Markov chain's stochastic state during stage $k$,
and let $P(X^{k}=x)$ be the probability that \corr{that}{} the chain is in the
...
in one particular state $y$\corr{,}{ and} then entering $x$. The overall
...
Given that \corr{}{the} long-term behavior of
Algorithm~\ref{alg:Iterative-scheme-to} shapes the predictions made with
\gls{matsim}, its characterization \corr{as}{in terms of} the stationary
...
are\corr{:}{the} demand $D$, \corr{}{the} network condition $C$ and \corr{}{the}
information $Z$. Of these, only \corr{}{the} information $Z$ qualifies as a
state variable:
\corr{if}{If} one knows $Z^{k}$, it is possible to draw the next day's travel
demand $D^{k+1}$\corr{,}{} based on \MyEqRef{eq:choice-model}, to insert this demand
into \MyEqRef{eq:network-loading-model} and obtain the network conditions
$C^{k+1}$ and \corr{}{to} finally use both $C^{k+1}$ and $Z^{k}$ to obtain an
updated $Z^{k+1}$ through \MyEqRef{eq:learning-model}.
This last step is what disqualifies $D$ and $C$ as state variables\corr{,}{}
because an evaluation of \MyEqRef{eq:learning-model} is impossible without having $Z$ in
the state space.

Letting $X^{k}=Z^{k}$, the transition distribution hence needs to express how
...
\end{eqnarray}
Each \corr{double sum argument}{argument of the double sum} represents the
probability of one particular \corr{given information $y$sequence, resulting travel demand
...
that the concrete iteration \corr{where}{in which} one evaluates the simulation
...
\MyEqRef{eq:learning-model} remains the same as $Z^{k-1}$. Practically, this
means that all plan scores stay unchanged.
% % \kai{@Gunnar: Habe das mal umgeschrieben, weil in der praktischen
% Implementierung die Agenten kein Memory haben; sie merken sich nur den
% geglätteten Score.} % \gunnar{ok}

More intuitively\corr{, this means that, even}{: Even} if the system returns
multiple times exactly to a state where it has been before, it \corr{is
improbable}{unlikely} that it does so in the same number of steps.
% % \kai{@Gunnar: pls chk the above.} % \gunnar{ok}
...
\corr{the system is in general not irreducible:}{irreducibility cannot be
postulated:}
% % The MATSim property that causes problems when it comes to irreducibility are
% the changes to the set of possible plans per agent:
...
guaranteed, \corr{these}{the resulting} process dynamics would be slow due to
...
likely to
% have % these properties if the plan choice sets of all agents are a priori %
% defined, meaning technically that all {}``plan innovation'' modules % are
% switched off and that only {}``plan selection'' modules are % used.
...
even though \corr{}{the} long-term choice distributions remain unchanged.
...
rigorous characterization of \corr{}{the outcome of} \gls{matsim}'s relaxation
process\corr{ outcome}{}.
...
Many of the models used in \gls{matsim} are stochastic\corr{: for instance,}{.
Examples are the discrete choice models used for} plan selection\corr{
random utility model}{} or the randomized selection of the next vehicle to enter a congested
...
The insertion of randomness represents \corr{}{the} uncertainty remaining in the
modeling.
...
To help intuition, one may think\corr{,}{} in the following\corr{,}{} of $y_{r}$
...
\end{eqnarray}
This expression merely sums up \corr{all simulation probability
outputs}{the probabilities of all simulation outputs} that exceed the threshold.
\item ...
\end{itemize}
%
...
Now recall \corr{initially asking certain questions about simulation
outputs}{that initially certain questions about simulation outputs were asked}.
The \MyEqRef{eq:define-question} (first row) represents exactly these questions
in a formal way--and \MyEqRef{eq:approximate-answer} (last row) provides a
simple method for computing answers to these questions\corr{, which}{. It} reads
as follows:
\begin{enumerate}
...
This long-term behavior was then related to \corr{iteration logic
properties}{the properties of the iteration logic} using the the Markov chain
formalism.
\gls{matsim} was phrased as such a chain, with its state space comprised of
\corr{nformation}{the information} available for replanning. This representation
...
fixed.
% ; and to (ii) provide insights into how the \texttt{ExpBetaPlanChanger} avoids
% network breakdowns.
