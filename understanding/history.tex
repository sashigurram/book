...
%
\kwaah{In the late 1990's transport science addressed the process of learning in the context of the new possibilities of 'intelligent transport systems'. It used various smoothing techniques to integrate data from iteration to iteration reflecting the tradition of the field. Examples include \cite{%
%
KaufmanEtc-91,%
...
\kai{Die Logik hier ist jetzt in etwa: Es gibt eine Liste, siehe Fig.~\ref{fig:dimensions}, und die bildet den roten Faden für die Arbeiten am IVT.  ``History'' ist insofern alles, was von dieser Liste bereits bearbeitet ist.  Ok so?  Sonst bitte anpassen, wobei ich gerne ``history'' und ``future'' ($=$ research avenues) auseinander halten würde.}
...
{\citet[][]{Axhausen_unpub_NYC_2014} \ah{add missing refs} \kai{was ist rot und was ist schwarz?}}
% ----------
...
(see Figure~\ref{fig:dimensions} \kwaah{The figure should go to the research avenues or the introduction maybe}\kai{Finde ich nicht zwangsläufig, s.o.}). 
...
The computational overhead costs of calculating the set of non-chosen alternatives becomes prohibitive at the scale for which \gls{matsim} is designed \kai{Kann man das so sagen, wenn CEMDAP oder TASHA das inzwischen hinbekommen?}, so alternative approaches were explored. 
%
