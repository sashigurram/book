...
constant network delays: In a \gls{ue}, the flow on the currently best routes
is increased at the cost of the other route flows (``best reply'' or ``best response''
choice), whereas for a \gls{sue} the flows are shifted towards the desired
...
then this is a dynamic \gls{ue} since the best reply dynamics means that no
...
Disaggregating an \gls{od} matrix into individual trip-makers allows to 
...
The notions of \gls{ue} and \gls{sue} carry over to the disaggregate case if the notion of an \gls{od} pair (or a commodity) is replaced by that of an individual \textbf{particle} ($=$ microscopic traveler).
...
An operational specification of a particle \gls{sue} results if one
...
If the iterations aim at an agent-based \gls{ue}, then choice set generation
...
The simulation of an agent-based \gls{ue} is possible by the following 
...
\citep[][]{recker-2001}. 
This constitutes an important technical motivation to switch to an agent-based \gls{sue}, 
...
manageable, in computational practice multiple agents need to make
...
The major criticism of the agent-based \gls{ue} is its lack of behavioral realism.
...
representation of the imprecision in the behavioral model.
...
\item \textbf{Selection:} Decrease the occurrences of ``bad''
...
interpreted as the choice set.  As in \gls{sue}, the choice set may be
...
which bears some similarity with fictitious play 
...
connection to the \gls{sue} definition by interpreting the synthetic
