...
%%All developments in this paper assume within-day dynamic behavior,
...
%%Sec.~\ref{sec:with-day-repl} will investigate some of its consequences.  
%---
The paper finishes with a conclusion in Sec.~\ref{sec:conclusion}.
...
We start by considering route assignment only. The generalization towards further choice dimensions will turn out to be a natural generalization of the route assignment problem.
...
The choice set generation implements a ``best reply'' dynamic.\todoNextRevision{transims?} This now requires to identify an optimal all-day plan for given network conditions. While the calculation of time-dependent shortest paths for UE route assignment is computationally manageable, the identification of optimal plans is far more difficult \citep[][]{recker-2001}. 
...
%Sec.~\ref{sec:extend-route-assignm}, this may look as
%follows:
\begin{lstlisting}{}
...
MATSim has, in the past years, gained some experience with the
approach described in the following paragraphs.

%%\todoKai{Wenn wir uns an die uebliche Notation halten wollen, dann
%%sollten wir im Folgenden $V$ fuer "systematic utility" verwenden.
%%$U=V+\varepsilon$ is die random utility. Weiss aber nicht, wie gut
%%das dann zu der sonstigen Matsim-Literatur passt. Gunnar}

\paragraph{Total utility}

The total score of a plan is computed as the sum of individual
contributions:
%%\todoKai{Replaced $n$ by $m$ because $n$ is already the agent
%%index. Gunnar}
%% ok. kai
\begin{equation}
...
At this point, both $\beta_{perf}$ and $t_{*,i}$ are fixed because
...
%%that is related both to the minimum
%%duration and to the importance of an activity.  %If the actual duration
% falls below $t_{0,i}$, then the utility contribution of the activity
% becomes negative, implying that the agent should rather completely
% drop that activity.  
%%As long as dropping activities from the plan is not allowed, $t_{0,i}$
...
\paragraph{Disutility of schedule delay}
...
is negative.
%%\todoKai{Warum steht $\beta_{tr}$ hier in Betraegen? Wenn es tatsächlich
%%positiv ist, dann geht es hier negativ ein ... Gunnar}
...
%%\todoKai{Gleiche Frage zu $\beta_{late}$. Warum die Betraege? Gunnar}
% 
% The utility function could be extended with additional terms. For
% example, one could think of tolls or parking costs that could be
% included into the score.

\todoNextRevision{work by arentze/timmermans 
  $\Rightarrow$ Da kenne ich mich nicht aus. Gunnar}

\todoNextRevision{w/o scores, the approach would still work w/o but ...}
...
%%, such as travelers being able to take their
%%own car from a location only if they brought it to that location
%%before.  
...
%Fig.~\ref{fig:plan-detailed}.\todoKai{missing figure}
...
%Fig.~\ref{fig:plan-detailed}, 
events may include the following (again
using XML as a language):
%%\todoKai{missing figure -- Ich weiss leider nicht
%%was fuer eine Zeichnung Du hier im Sinn hast. Gunnar}
\begin{lstlisting}
...
Note that the term ``microscopic'' refers to the \textbf{resolution}
...
%%\subsection{Demand vs.\ supply vs.\ ``physics''}

%%\todoKai{The ``physical'' module is the one that computes the scoring
%%  based on the performance in the synthetic reality; not sure in how
%%  many words we should state this in particular since there is some
%%  disagreement between ``supply simulation'' and ``physical
%%  simulation''.  (For me, the network loading is what the economists
%%  call ``technology''.  A supply simulation simulates a traffic
%%  management center that decides about management strategies, or a
%%  political system that decides about supply expansions.)  Es gibt
%%  auch einen geplanten Abschnitt ``Demand vs supply vs physics'', wo
%%  das diskutiert werden koennte. Kai
%%  $\Rightarrow$ Wir meinen mit Sicherheit dasselbe. Ich habe den Eindruck,
%%  dass die meisten DTA-Leute supply = network loading setzen, was hier ja
%%  noch passt. Aber wenn Du hier noch detaillierter werden moechtest, dann
%%  sollten wir zu Deiner Terminologie uebergehen. Gunnar}

%%\subsection{Risk}

%%\subsection{Consequences in the economic interpretation (and things
%%  that are conventionally probably wrong)}

%%route assignment w/ times ... econ analysis with generalized costs
%%... ???  But w/ rule of half this still works???

%%\subsection{Feedback to upstream models}

%%\todoKai{is this related to ``econ interpretation''?  Gunnar --- Ich
%%  denke vor allem an ``feedback to urbansim''.  Kai
%%  $\Rightarrow$
%%  Es sollte erwaehnt werden, dass das feedback mit den agents zumindest prinzipiell
%%  leicht auf land use ausgedehnt werden kann. "econ interpretation" scheint mir da
%%  aber der uebernaechste Schritt. Aber gehoert dies denn in die "theo" section? Gunnar}
...
%%\todoKai{Repair citations; see mail. Gunnar}
