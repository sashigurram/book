...
\gunnar{Fwd. pointer to resp. research avenue section.}
...
integers. \todoNextRevision{wikipedia linear programming integer unknowns refers
  ...
  too early).  Nemhauser integer programming?} \gunnar{Ich meine mich zu
  erinnern, dass interior point f\"ur \emph{non-integer} LPs non-exponential ist.}
...
% </plan>
% \end{lstlisting}
Specifically, MATSim plans as exemplified in \gunnar{Section reference.} maintain the structure of 
DTA in terms of the triple
(origin, departure time, destination). However, different from
DTA, all activities are chained together.
...
\begin{equation}
P(\text{old} \to \text{new}) \propto \max[ 0, e^{\beta \, (S_{\text{new}} - S_{\text{old}})} - 1]\ .
\end{equation}
...
\begin{equation}
e^{\beta \, (S_\text{new} - S_\text{old})} - 1
\approx\beta \,  (S_\text{new} - S_\text{old})\ ,
\end{equation}
...
\gunnar{Weiss gar nicht, wieviel diese Gleichungen helfen. Vielleicht einfach sagen,
dass es einen Auswahlbias in Richtung besserer Pl\"ane gibt?}
...
This section discusses MATSim's coevolutionary algorithm for simulating
...
provided in Sections \gunnar{REF:choice models in MATSim} and \gunnar{REF: CBA}.
...
choice theory; see Sections \gunnar{REF: choice models in matsim} and 
\gunnar{REF: research avanues}.
...
\gunnar{Add explanation why so that this can be referred to in
the MC chapter.}
...
presumably realistic. Sections~\gunnar{REF to discrete choice models}
attempts to relate this to the noise specifications in random utility 
modeling; see also Section~\gunnar{REF research avenues} for further
...
Overall, this chapter has phrased MATSim's AI-based mechanisms in terms of 
DTA. Observing the structural similarity between the ``classical" DTA 
problem and the more recent agent assignment problem, the presentation has 
focused on the algorithmic issue of how to find solutions of these problems.
This is complemented by the subsequent Sections \gunnar{REF}, which mostly
discuss modeling (descriptive) aspects of MATSim.
