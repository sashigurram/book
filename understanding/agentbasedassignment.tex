...
It is common knowledge \gunnar{Perhaps not common enough to not give a
reference?} that situations may occur where mixed strategy equilibria exist;
these are equilibria, where participants draw between different fixed strategies
...
deconstructed by agents once network conditions are given---a great computational advantage. 
The supply model, on the other hand, deals with congestion, which is, by definition, a result of all travelers' 
physical interactions. Modeling large urban areas requires dealing with millions of travelers 
and an operational supply simulation must be able to load all of these travelers on the network in 
...
\textbf{Choice set generation:} For every agent,
  generate what would have been best in the previous iteration.
  This does not concern just the route,
...
computationally manageable, identification of optimal plans is far more difficult 
\citep[][]{recker-2001}. 
...
robust if plan performance is directly taken directly from the network
loading, instead of relying on the best reply computation prediction;  
...
Truncation at zero is not necessary because the term $S_{best} - S_{old}$ cannot become negative.
%}
...
Clearly, any fixed point of such iterations is a \gls{ue}, since, at the
...
at the fixed point, in the above formulation on the $\mu$: all
other things equal, making $\mu$ smaller makes the fixed point more
stable, but slows down convergence. 
%
These observations hold not only in transporation \citep[e.g.,][]{watling-2003},
...
spillback across many links has apparently been shown to be an nevitable
...
%\kai{Danke!}
One disadvantage: with \gls{msa}, the
switching rate does not depend on the magnitude of the expected
improvement, which could means slow(er) convergence.  An advantage
...
an explorative learning algorithm can be oriented either as
...
for mainstream discrete choice theory \citep{BenAkivaLerman_1985}.
...
approach: each person has a plan collection, which may be
...
see Chapters~\ref{ch:discretechoice} and Section~\ref{sec:choicesets}.
...
Equation~(\ref{eq:2}) directly, but instead use a switching process that
...
\citep{ben-akiva-1999} in choice probabilities.
...
although we have found is that this may not be as simple as it seems
...
This chapter takes the inductive approach and argues that one can make
...
computation of scoring needs to take the effects of these
...
viewpoint of the iterative solution procedure, rather than the
problem definition.
