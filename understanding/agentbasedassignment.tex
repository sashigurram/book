...
Solution of the traffic assignment problem describes an 
...
defined by a fixed point: find a demand pattern generating network 
conditions that, in turn, cause the same demand pattern to re-appear. This 
formulation is operationally important because it encourages a straightforward 
way of calculating an equilibrium by alternately evaluating the demand 
...
The constant demand $q^{od}$ in a \gls{od} relation $od$ splits up among a set of routes $K^{od}$. 
Denote the flow on route $k\in K^{od}$ by $r^{od}_k$, where $\sum_{k\in K^{od}} r^{od}_k = q^{od}$.

Most route assignment models either specify a \gls{ue} (Nash, Wardrop) ,
or a \gls{sue}.  
...
the routes such that a \gls{sue} is
...
constant network delays: in a \gls{ue}, the flow on the current best routes
is increased at the cost of the other route flows (``best reply'' or ``best response''
choice), whereas for a \gls{sue},  flows are shifted towards the desired
...
this is then a dynamic \gls{ue}, since best reply dynamics mean that no
traveler (no \gls{od} flow) can unilaterally deviate to a better route.  The
...
assignment of one vehicle to every trip-maker in the microscopic traffic flow 
simulation.
...
one reduces the traveler number to $\varepsilon \rightarrow 0$ and
...
A \textbf{particle \gls{ue}} may be defined as a system state, where no
...
solutions in its discrete version.  That is, the particle \gls{ue} is
...
It is common knowledge that situations may occur where mixed strategy
equilibria exist; these are equilibria, where participants draw
...
An operational particle specification \gls{sue} results if one
assumes that travelers filter out the random fluctuations from what
they observe and base their decisions only on average route
costs:
\begin{equation}
...
represent not only mean network conditions,
...
draws from the choice distribution have a
...
\emph{Thus, using a stochastic, instead of a deterministic, network
loading makes little difference.}  
...
With stochastic network loading, additional iterative dynamics aspects
need to be defined.  For example, a ``best
...
need to be modeled, but also carries a set of natural constraints
along, which again reduce the solution space. Most notably, the
destination of one trip must be the origin of an individual's subsequent trip 
and an agent (synthetic traveler) must arrive before it departs.
...
If  iterations aim at an agent-based \gls{ue}, then choice set generation
and choice should implement a ``best reply'' logic; some
'optimal' plans are calculated and assigned to the agents. This
is emphatically not an easy task.  
