...
%\atend{read part III from A-Z} \ah{done, see also mail by Kai}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% \atend{permission from original publisher?}
% we have that (by email).
...
\corr{Solution}{A solution} of the traffic assignment problem describes an 
...
defined by a fixed point: \corr{find}{Find} a demand pattern generating network 
conditions that, in turn, cause the same demand pattern to re-appear. This 
formulation is operationally important because it \corr{encourages}{motivates} a
...
The remainder of this chapter places \gls{matsim} into this \gls{dta} framework.
Section~\ref{sec:agent-based-DTA-from-routes-to-agents} starts out from the
static and macroscopic \corr{route flow assignment}{assignment of route flows}
...
interpretation as a model of day-to-day replanning; see also Section~\ref{sec:transients-vs-learning}.

We start by considering route assignment only. The generalization towards further 
choice dimensions will 
turn out to be \corr{a natural generalization of the route assignment
problem}{rather straightforward}.
...
The constant demand $q^{od}$ in \corr{a}{an} \gls{od} relation $od$ splits up
among a set of routes $K^{od}$.
Denote the flow on route $k\in K^{od}$ by $r^{od}_k$, where $\sum_{k\in K^{od}} r^{od}_k = q^{od}$.

Most route assignment models either specify a \gls{ue} \corr{(Nash, Wardrop),}{}
or \corr{a}{an} \gls{sue}.  
...
An alternative, frequently-used approach is to distribute the demand onto
the routes such that \corr{a}{an} \gls{sue} is
...
Defining the network loading as more on the ``physical'' side of the system, 
...
constant network delays: \corr{in}{In} a \gls{ue}, the flow on the current
best routes is increased at the cost of the other route flows (``best reply'' or ``best response''
choice), whereas for \corr{a}{an} \gls{sue},  flows are shifted towards the
...
way that dampens \corr{}{the} iteration dynamics. See below for more
...
\gunnar{Started a new paragraph here.}
...
these new routes and \emph{if} these iterations reach a fixed point\corr{;}{,
then} this is then a dynamic \gls{ue}\corr{,}{} since best reply dynamics mean
...
Destination choice and elasticity in demand apply naturally to 
...
%ok. kai
%
Arguably,\corr{ in the dynamic case,}{} this re-interpretation is behaviorally
more plausible \corr{}{in the dynamic case}.
...
\corr{}{the} assignment of one vehicle to every trip-maker in the microscopic
...
one reduces the traveler \corr{number}{size from one} to $\varepsilon
...
\corr{those previously}{what was shown before}:
...
A \textbf{particle \gls{ue}} may be defined as a system state\corr{,}{} where no
...
\gunnar{I have reorganized the reminder of this paragraph.}
...
Further, \corr{this is structurally similar to}{the complexity of finding a
solution increaces similarly to} the situation \corr{that}{where} linear
...
} The particle \gls{ue} is hence deliberately not searching for\corr{, say,}{}
an integer approximation of the continuous solution.
%  \citep{karmarkar1984}.
% % \todoNextRevision{wikipedia linear programming integer unknowns refers %  
% to the interior point method as non-exponential solution method of %   linear
% programming.  There should be some book (Garey and Johnson is %   too early). 
% Nemhauser integer programming?} %   \gunnar{Von mir aus k\"onnen wir den
% letzten Satz gerne weglassen.
...
% \kai{Meiner Erinnerung nach stammt der NP-hard-Satz ursprünglich von mir.  Du
% hattest dann wohl nach einer Quelle gesucht und dabei die wikipedia-Aussage
% gefunden. -- Beim genauen Lesen ist das aber eigentlich kein Widerspruch:
% linear programming in real variables is non-exponential; wenn die Lösungen nur
% noch integer sein dürfen, wird es NP-hard. -- Also fehlt nach wie vor nur noch
% die Referenz.  Gunnar, wenn Du soweit einverstanden bist, dann schreib' bitte
% kurz ``ok''; vielleicht finden wir dann noch eine Referenz.} % \gunnar{OK.
% Karmarkar scheint hier die Schl\"usselreferenz zu sein:}

\corr{It is common knowledge that situations}{Situations} may occur where mixed
...
\corr{even}{Even} if they themselves play fixed strategies, they need to
...
stationary choice distribution\corr{;}{ and where} the resulting distribution
of traffic conditions re-generates that choice distribution.

An operational \textbf{particle \corr{specification}{} \gls{sue}}
\corr{}{specification} results if one assumes that travelers filter out the
...
but also their variability, due to \corr{}{the} individual-level
route sampling.
...
This is especially true if day-to-day replanning is modeled\corr{ where}{, in
which case} draws from the choice distribution have a
behavioral interpretation as the actual choices of the trip makers
on a given day (but see also Section~\ref{sec:transients-vs-learning}).
...
This, however, makes \corr{}{the} output from the network loading effectively
...
Making \corr{}{the} network loading stochastic simplifies the implementation of
...
With stochastic network loading, additional \corr{iterative dynamics
aspects}{aspects of the iterative dynamics} need to be defined.  For example, a
...
example, the ``best reply'' can include optimal departure \corr{times}{time
choice} \citep[e.g.,][]{METROPOLIS,EttemaEtcRoutes-timesIatbr03} \corr{,}{} or
...
choice dimensions of \corr{a daily}{an all-day} travel plan can now be jointly
...
destination of one trip must be the origin of \corr{an individual's}{the same
agent's (synthetic traveler's)} subsequent trip and an agent \corr{(synthetic
traveler)}{} must arrive before it departs.
...
There is so significant conceptual difference between the network loading of
...
stationary choice distribution \corr{;}{and where} the resulting distribution of
...
If \corr{}{the} iterations aim at an agent-based \gls{ue}, then choice set
...
is \corr{emphatically not}{anything but} an easy task.  

The disaggregate counterpiece of \corr{a}{an}
...
Summarizing, we have now arrived at a \corr{}{fully disaggregate} dynamic DTA
...
\corr{deconstructed by agents}{decomposed by agent} once \corr{}{the} network
...
computationally manageable, \corr{}{the} identification of optimal plans is far
...
robust if plan performance is directly taken \corr{directly}{} from the network
...
Clearly, any fixed point of such iterations is a \gls{ue}\corr{,}{} since\corr{,
at the fixed point,}{} no switching takes place \corr{}{at the fixed point},
...
at the fixed point, in the above formulation on the $\mu$: \corr{all other
things}{All else} equal, making $\mu$ smaller makes the fixed point more
...
\corr{nevitable}{inevitable} source of multiple Nash equilibria
\citep[][]{daganzo-1998}.
%%; also see
...
%\kai{Danke!}
One disadvantage\corr{:} with \gls{msa}\corr{,}{is that} the
switching rate does not depend on the magnitude of the expected
improvement, which could \corr{means}{mean} slow(er) convergence.  An advantage
...
These considerations do not only lead naturally to the agent-based \gls{sue};
...
an explorative learning algorithm can be \corr{oriented}{interpreted} either as
...
\corr{for}{in terms of} mainstream discrete choice theory
...
For \corr{}{the} construction of new solutions, two operators are often
...
Note that this approach is really quite congruent with the \gls{sue}
approach: \corr{each}{Each} person has a plan collection, which may be
...
every plan a score-based probability to be selected, \corr{}{which becomes}
...
see \corr{Chapters}{Chapter}~\ref{ch:discretechoice} and
...
A possible choice algorithm is the following: \corr{for}{For} persons with
...
achieved by using a switching probability from $i$ to $j$ of \corr{}{the form}
\begin{equation}
...
\item One could replace Equation~(\ref{eq:3}) by a threshold-based
 ...
 it may be more consistent with discussion of
  ...
  lead to a change in behavior. 
	\gunnar{Warning -- what means ``consistent with discussion of
  project appraisal''?}
...
\citep{ben-akiva-1999} in \corr{choice probabilities}{the choice model}.
...
a particular iteration might display \corr{}{a} ``network
...
%% \todokai{clarify score convergence with newer MSA option}
...
although we have found \corr{is}{} that this may not be as simple as it seems
...
This chapter takes \corr{the}{an} inductive approach and argues that one can
...
\corr{computation of}{} scoring needs to take the effects of these
additional choice dimensions into account \citep[also
  see][]{Balmer_PhDThesis_2007}.  Given evolutionary game theory, it is quite
obvious how to do that: \corr{one}{One} has to extend the cost
...
caused by \corr{}{the} expansion of the choice dimensions,
...
Most of this chapter discusses simulation/computer implementation issues.
...
problem.\footnote{%
%  % \gunnar{Wasn't there somewhere on the web site a tutorial on % ``running
% MATSim as a standard DTA''? That would fit well in here, I think.} \kai{ok. 
% Not much of a tutorial, but at least an example plans file and some text with
% it.  Will do when I get around.}  \kai{See below.}
...
\usebox{\tripAsPlan}.
} The presentation \corr{}{has} focused on the algorithmic issue of how to find
