...
MATSim comes with a number of {}``mobsims''. These differ from the
...
At the heart of MATSim's traffic flow model is the flow-density relationship
(also called fundamental diagram) shown in Figure~\ref{fig:Fundamental-diagram}
and subsequently abbreviated as {}``FD''. Given a long, homogeneous
...
The FD is defined as the minimum of a sending function $S(\varrho)$
...
out to be fairly constant; a vehicle length of 7.5\,m and a time
gap of 2\,s leads to a value of 13.5\,km/h. \note{Can it be set somewhere, or is there a hardcoded value?}

The considered FD alone applies only in stationary conditions, where
it predicts that (i) flow increases linearly with density at low densities
(i.e. in uncongested conditions); (ii) flow decreases linearly with
density at high densities (i.e. in congested conditions); and (iii)
...
FD into a continuity equation (which intuitively models vehicle conservation,
in the sense that vehicles cannot vanish or spontaneously appear on
a road segment without on- and off-ramps). This leads to the Kinematic
Wave Model of traffic flow \citep[KWM;][]{lighthill-1955,richards-1956},
...
densities prevailing and FDs applying up- and downstream of that interface,
...
The remainder of this chapter expresses MATSim\textquoteright{}s link
model (Section~\ref{sec:Link-model}) and its node model (Section~\ref{sec:Node-model})
in terms of the sending and receiving function framework of the KWM.
...
MATSim also needs to compute these quantities. The way in which it
...
downstream interface. In terms of the continuum model (i.e. one that
...
MATSim's vehicle-discrete perspective, the cumulative inflow (outflow)
...
The connection to MATSim can now be made explicit by labeling the
...
A mobsim that implements the rules \MyEqRef{eq:UQ-rek}, \MyEqRef{eq:DQ-rek}
and \MyEqRef{eq:R-matsim}, \MyEqRef{eq:S-matsim} implements a KWM-consistent
...
the \note{REF single-queue mobsim(s)} behave like a KWM with an extremely
...
All mobsims in MATSim implement the same node model. Somewhat surprisingly,
this node model can be traced back at least to \note{when?}, whereas
the literature based on which its consistency with the KWM can be
established is only a few years old \citep{tampere-2010b,floetteroed-2011a,corthout-2012}. 

Nodes in MATSim have no spatial dimension; they merely connect up-
and downstream links. \citet{tampere-2010b} specify a set of requirements
for a (continuum) node model to be consistent with the KWM. They require
...
instance, if it is specified that 20 percent of the outflow of link
$i$ shall turn into link $j$, then the amount of flow that actually
advances from link $i$ into link $j$ shall indeed be 20 percent
...
advancement of a queueing vehicle is not affected by the vehicles
behind it.
...
Now consider the code documentation of MATSim's \texttt{queuesim.QueueNode.moveNode}
(as of December 2014):
\begin{quote}
...
There are two aspects of the MATSim node model that is not reflected
...
\end{itemize}
The relative simplicity of MATSim's intersection logic may be refined
