...
Sections~\ref{sec:using-mobsims},~\ref{sec:deqsim}); the most important 
are the so-called \SINGLEQUEUESIM\ and 
...
There are other similar simulation packages rooted in the same underlying modeling concepts 
\citep[][]{mahut-2007,zhou-2014}. 

The flow-density relationship (also called \gls{fd}), shown in Figure~\ref{fig:Fundamental-diagram}
is at the heart of \gls{matsim}'s traffic flow model.
...
backward shockwave speed $w$. Maximum velocity is an observable
parameter that can be set in the network file (\texttt{freespeed}
attribute of the \texttt{link} element). Maximum density equals
...
of the \texttt{link} element), otherwise. Backward wave speed turns
out to be the ratio of vehicle length to the safety time
...
instantaneous flow across any interface, possibly with different
...
receiving function then allows expression of sending and receiving flows
...
which, in its implementation as of December 2014, is only limited
...
\end{eqnarray},
which represents only total number of vehicles in the link.
...
literature citing its consistency with the \gls{kwm} 
is only a few years old \citep{tampere-2010b,floetteroed-2011a,corthout-2012}. 
...
downstream are not exceeded. This is explained in Section~\ref{sec:Link-model}.
...
by a competing upstream link: in practical terms, a right-of-way specification.
\end{enumerate}
\citet{floetteroed-2011a} specify an {}``incremental node model''
that satisfies these requirements, also providing an intuitive,
computationally efficient solution algorithm. In each simulation
...
There are two aspects of the \gls{matsim} node model not reflected
by the above code comment.
\begin{itemize}\styleItemize
\item Sending flows of an upstream link may be limited by by an outflow
capacity below the flow capacity \MyEqRef{eq:flow-cap} of
that link: for instance, to approximate capacity reduction resulting
...
in many ways; for instance, turning pockets may be added and conflicts
within intersections may be modeled (\cf Chapter~\ref{ch:signalslanes}). 
